...
3. Updated `crates-archive/` and `CHECKSUMS.toml` per §5.5.
...
- The unmodified `.crate` artifacts are committed to `crates-archive/`. Cargo is configured with a local-registry source pointing at this directory; builds run `--offline --frozen --locked`. CI fails if any build reaches the network or if `crates-archive/` and `Cargo.lock` disagree.
...
Crate artefacts are stored as the unmodified `.crate` tarballs in `crates-archive/<name>-<version>.crate`. Cargo is configured to consume them via a local-registry source. We do not commit the unpacked `vendor/<name>-<version>/` directory tree: the `.crate` is the authoritative artefact whose hash we control, and unpacked sources drift in subtle ways (line endings, permissions, extended attributes) that make a stable hash hard to reproduce across hosts.
...
2. Computes SHA-256 of every `.crate` in `crates-archive/`.
3. Fails if any `.crate` is missing from the manifest.
4. Fails if any manifest entry is missing from `crates-archive/`.
...
- Confirms byte-equality with what Cargo placed in `crates-archive/`.
...
2. Run `tools/audit-helper fetch <crate> <version>` to fetch the new `.crate` files into `crates-archive/`. The helper refuses to overwrite existing files; updates are explicit removals and re-fetches.
...
5. Commit `CHECKSUMS.toml`, `crates-archive/<name>-<version>.crate`, and the updated `Cargo.lock` together. No splits.
...
- **Registry compromise.** If crates.io is compromised after a crate was audited, the registry's serving of different bytes under the same version does not change `crates-archive/`. CI fails against the on-disk artefact. This is the most important property.
...
- Has a stable schema documented in `architecture/02-3-audit-schema.md`.
