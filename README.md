...
Pre-1.0, on a versioned cadence — latest [0.3.0 "dynamic spawn"](https://github.com/projectkennel/projectkennel/releases) plus the [0.3.1](CHANGELOG.md) installer fix; **0.4.0, the service-mesh release, is in progress**. The threat catalogue ([v0.3](docs/design/THREATS.md)) and the design document are publishable, and the **reference runtime and policy compiler are implemented** — not just designed.

Working today (kernel 6.17, Landlock ABI ≥ 6; see [BUILD-ENV.md](docs/design/BUILD-ENV.md)) — **the full vertical runs unprivileged**, proven end-to-end as the ordinary operator with no `sudo`, each slice exercised by the real installed toolchain in the policy-suite e2e:

- **The spawn:** an identity-mapped user namespace, a double-fork so the workload is PID 1 of its own PID namespace, the constructed-`$HOME` view via `pivot_root` (non-granted paths are *absent*, not merely denied), a synthetic `/etc`, an allowlisted `/dev` with host-device passthrough, a fresh `/proc` + private `/tmp`, Landlock filesystem + network rules with abstract-unix/signal scoping, a seccomp denylist, `PR_SET_NO_NEW_PRIVS`, and cgroup join. Construction is unprivileged and per-task, and **cheap enough to be disposable** — measured at **≈ 3.7 ms median (≈ 4.4 ms p90), workload-independent**, with teardown **≈ 0.7 ms** (the build→workload-exec span from the daemon trace, 20 iterations inside one control kennel; [`spawn-spinup.sh`](src/tools/spawn-spinup.sh) / [`spawn-latency.sh`](src/tools/spawn-latency.sh), kernel 6.17). That floor is what lets confinement be per-task rather than per-session.
- **Dynamic spawn (§7.12):** a confined workload (an agent) instantiates a *sibling* kennel from a signed, `@`-pinned template it holds a `[spawn]` grant for, over the per-kennel binder — the capability floor is the signed template's, the caller chooses only what the template's manifest opens (`kennel run <template@version> [field=value]… -- <argv>`), bounded by an atomic `max_instances` fork-bomb ceiling. Agent↔tool composition with no daemon-side protocol parsing.
- **The cross-kennel service mesh (§7.13):** kennels `[provides]`/`[consumes]` named capabilities; `kenneld` derives a catalogue (a projection of signed policy, never authored central state), brokers an af-unix connector through a host-owned rendezvous point, socket-activates an `ondemand` provider on first consume and idle-reaps it when no consumer runs — deny-by-default, reserved-namespace-gated. The binder IPC (§7.1) it rides — kenneld as each kennel's node-0 context manager — is the same path.
- **Confined GUI (§7.14):** a GUI app reaches a real desktop through a per-kennel **nested compositor** run as a service kennel (bring-your-own `cage`/Weston/sway, host-independent — proven on stock GNOME), brokered over the mesh; the app's `wl_registry` is the inner compositor's, the host's clients absent by construction. The render/display leg is built; the interactive file broker is fenced post-0.4.0.
- **OCI substrate (§7.11):** `kennel oci` boots a digest-pinned, unpacked OCI image as the kennel root (the image fetched/unpacked by stock `skopeo`/`umoci` inside a confined `oci-fetch` kennel, never the daemon), under the same Landlock/seccomp/egress confinement.
...
- **Egress & IPC:** four network modes (`none` / `constrained` / `unconstrained` / `host`) — a per-kennel SOCKS5/HTTP proxy with a cgroup-BPF fail-closed allowlist and a per-kennel JSONL audit log, an inbound-bind mirror, an `AF_UNIX` socket shim, a mediated **D-Bus facade** (`org.projectkennel.IDBus`, §7.7 — per-message method filtering, no raw bus into the kennel), and a per-user SSH re-origination bastion (the workload holds no key or agent socket).
...
- **The `kennel` CLI:** `run`/`attach`/`stop`/`list` (the listing carries the cross-kennel mesh view), `oci` (build/run an image substrate), `review` (re-pin the trust manifest), `keygen`/`subkennel`/`audit`/`daemon-reload`, and a `kennel policy` group — `compile` (resolve a source policy + its templates into a signed, byte-pinned settled policy), `validate`, `sign`, `list`/`show`/`edit`/`generate`, `lint`, `risks`, `diff`, `upgrade`. One name, two contexts: host-side it is the operator command, and inside a spawn-capable kennel the same `kennel` dispatches `run`/`caps` over the binder (W10). An interactive `run` is **detachable** — kenneld owns the controlling pty and brokers it, so `Ctrl-\ d` detaches without ending the workload and `kennel attach <name>` reconnects (the tmux/`docker attach` model, no `setns`). End-to-end ed25519 trust (templates, fragments, and the settled artefact), a `kennel.lock` byte-pin, and a control-plane version handshake that refuses a CLI/daemon schema skew at the boundary. The shipped [templates](templates/) and composable [fragments](fragments/) are signed under the maintainer key `kennel-maint-2026` (verify with `kennel policy validate --require-signed` against [keys/](keys/)).
...
X11 is **cut**, not deferred: 0.3.0 removed it, and confined GUI replaces it with the host-independent nested Wayland compositor above (§7.14). Deferred (designed, not yet built — see [docs/architecture/08-as-built-notes.md](docs/architecture/08-as-built-notes.md) §8.1): `fs.scrub`/`fs.home.sanitise`; the cross-*instance* binder relay (a kennel publishing a rich binder interface to others — the `binder-connector`/`dbus-name` mesh shapes are schema-typed but only the `af-unix` handoff is brokered today); confined GUI's interactive file broker (fenced post-0.4.0); and the minimal-view-floor tightening of the default `/usr`,`/var` visibility. The audit subsystem is complete at the userspace level (kernel-side BPF and LSM events report via the kernel's own ring buffer / `dmesg` by design, not this writer). Checksum verification is enforced today by the shell witness `src/tools/verify-checksums.sh`; a Rust twin is an optional §5.5.1 call (contingent on vendoring `sha2`), not a gap.
...
| Artefact | Size |
|---|---|
| Design docs ([`docs/design/`](docs/design/), 32 files) | ≈ 111,000 words |
| Architecture docs ([`docs/architecture/`](docs/architecture/), 20 files) | ≈ 79,500 words |
| Implementation — Rust (26 crates, tests included) | ≈ 33,800 SLOC |
| Implementation — BPF (C: 8 programs + 3 shared headers) | ≈ 545 SLOC |
| `unsafe` Rust — quarantined to five small crates | ≈ 4,700 SLOC, ~120 `unsafe` blocks |
