...
**The daemon runs unprivileged; one small file-capped helper does the rest.** `kenneld` is an ordinary user process — it runs as you, holds no standing privilege, and is the unprivileged orchestrator that schedules, brokers, resolves, and verifies the signed policy. The sandbox — user, mount, PID, IPC, and network namespaces, `mount`, `pivot_root`, the constructed view — is built by a single small **file-capped privhelper** that is deliberately frugal with the privilege: it drops its effective uid to the operator *across* the `clone`, so the workload's **user namespace is owned by the operator, not root** (the bubblewrap-equivalent mechanism — `CAP_SYS_ADMIN` only *inside* that namespace), and raises euid to 0 only briefly to write the kennel's identity maps (the `0 0 1` host-root line plus the operator and any policy-granted supplementary-group lines), then drops back. The few host-global operations a user namespace genuinely cannot reach — the per-kennel loopback mirror, the egress BPF (`host` mode only), the exclusive-bind over-mount — are delegated to single-purpose sub-helpers, each carrying just its one capability. There is no `sudo` anywhere in the spawn; privilege is concentrated in that one small, audited factory (plus its narrow sub-helpers), and the namespaces it builds are operator-owned.
...
- **Confined GUI.** X11 is **cut**, not deferred. A GUI app reaches a real desktop through a per-kennel **nested compositor** run as a service kennel — bring-your-own `cage`/Weston/sway, unpatched and host-independent (proven on stock GNOME) — brokered over the mesh behind a single tagged host-compositor leg. The app's `wl_registry` is the *inner* compositor's; the host's other clients are absent by construction, so the app draws and gets its own window without holding a raw host-compositor socket or any filesystem grant. The render/display leg is built and the session comes up as a windowed desktop out of the box; the interactive file broker is declined on principle — an in-session portal is its honest home, not a host reach-through.
