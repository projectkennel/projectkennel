...
- **Confined GUI.** X11 is **cut**, not deferred. A GUI app reaches a real desktop through a per-kennel **nested compositor** run as a service kennel — bring-your-own `cage`/Weston/sway, unpatched and host-independent (proven on stock GNOME) — brokered over the mesh behind a single tagged host-compositor leg. The app's `wl_registry` is the *inner* compositor's; the host's other clients are absent by construction, so the app draws and gets its own window without holding a raw host-compositor socket or any filesystem grant. The render/display leg is built; the interactive file broker is fenced post-0.5.0.
