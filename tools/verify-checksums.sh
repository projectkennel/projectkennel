...
#
#   1. every crates-archive/*.crate is recorded in CHECKSUMS.toml,
#   2. every CHECKSUMS.toml entry has its .crate in crates-archive/,
...
declare -A claimed=()      # crates-archive filename -> 1 (matched a manifest entry)
...
while IFS=$'\t' read -r name ver sha; do
	...
	if [ ! -f "$file" ]; then
		err "manifest entry $name $ver has no $name-$ver.crate in crates-archive/"
		continue
	fi
...
if [ -d "$ARCHIVE" ]; then
	while IFS= read -r path; do
		...
		if [ -z "${claimed[$base]:-}" ]; then
			err "crates-archive/$base is not recorded in CHECKSUMS.toml"
		fi
