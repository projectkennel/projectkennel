...
|---|---|---|
| `kennel-syscall` *(allow; owns the Landlock bindings + the spawn hook)* | The single point for namespaces, mounts, Landlock/seccomp, capabilities, credentials, and child spawning. Delegates `unsafe` to vetted crates (nix, seccompiler). Owns two `unsafe` sites, each §4-commented: the Landlock syscall wrappers (`src/landlock.rs`), a deliberate exception to keep `syn`/proc-macros out of the privileged TCB; and one `CommandExt::pre_exec` call (`src/spawn.rs`) registering the post-`fork`/pre-`execve` seal hook, wrapped here so `kennel-spawn` stays `#![forbid(unsafe_code)]`. | ~1500 lines (reviewable in one sitting) |
