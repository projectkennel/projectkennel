...
use crate::{default_key_dir, is_valid_key_id};
...
pub fn keygen(args: &[String]) -> Result<ExitCode, String> {
    // `keygen migrate` (the 0.5.0 raw-base64 → OpenSSH converter) is gone with the
    // legacy format; refuse it by name so it cannot fall through as a key id.
    if args.first().is_some_and(|a| a == "migrate") {
        return Err(
            "kennel keygen migrate was removed in 0.6.0 with the legacy raw-base64 key \
             format — regenerate with `kennel keygen <key-id>`, or convert once with \
