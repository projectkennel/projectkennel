...
//!
//! The test workload for the `mesh-roundtrip` policy-suite case, in two modes:
...
fn main() -> ExitCode {
    ...
    let Some(path) = args.next() else {
        eprintln!("facade-mesh-probe: usage: <serve|connect> <path>");
    ...
    match mode.as_str() {
        ...
        other => {
            eprintln!("facade-mesh-probe: unknown mode `{other}` (serve|connect)");
            ExitCode::FAILURE
        }
    }
}
