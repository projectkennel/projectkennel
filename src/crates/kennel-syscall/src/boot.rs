...
//! # Why
//!
//! `kennel-init` pulls its supervision-half over binder node 0, which `kenneld` claims by opening
//! the kennel's binderfs via `/proc/<init>/root`. That open only succeeds once `kennel-init` has
...
//! cannot gate the *exec*; it gates the *pull*, from inside `kennel-init`, over a socket the
//! factory hands it at [`BOOT_SYNC_FD`]:
//!
//! 1. `kennel-init` execs, then sends [`READY`] and **blocks** before opening the bus.
//! 2. `kenneld` (holding the other end) sees `READY`, opens the now-reachable binderfs, claims
//!    node 0, then sends [`GO`].
//! 3. `kennel-init` wakes and pulls — the context manager is already serving (first-try success).
//!
...
/// Borrow a raw descriptor for the duration of one sync call. Safe for our callers: `kennel-init`
...
/// The fixed descriptor at which the factory places `kennel-init`'s end of the boot-sync socket
/// (a `SOCK_SEQPACKET` pair), inherited across the `fexecve` — the sibling of
/// [`crate::pty::PTY_RETURN_FD`].
pub const BOOT_SYNC_FD: RawFd = 4;

/// `kennel-init` → `kenneld`: "I have `fexecve`'d; my binderfs is reachable via `/proc/<me>/root`."
const READY: u8 = 1;
/// `kenneld` → `kennel-init`: "node 0 is claimed and serving — pull now."
const GO: u8 = 2;

/// `kennel-init` side: announce we are up ([`READY`]) and block until `kenneld` confirms the bus
...
/// `kenneld` side: wait for `kennel-init` to announce it is up ([`READY`]).
///
/// # Errors
///
...
pub fn await_init_ready(fd: RawFd) -> io::Result<()> {
    match read_byte(borrow(fd))? {
        Some(READY) => Ok(()),
        _ => Err(io::Error::other(
            "boot-sync: kennel-init did not report ready",
        )),
    }
}

/// `kenneld` side: tell `kennel-init` the bus is live ([`GO`]) — call after claiming node 0.
///
/// # Errors
///
