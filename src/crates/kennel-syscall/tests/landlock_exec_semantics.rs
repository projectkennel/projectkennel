...
use kennel_syscall::landlock::{abi_version, AccessFs, Ruleset};
...
fn landlock_fs_execute_gates_libraries_not_just_the_binary() {
    // Establishes the kernel fact the exec-allowlist model (kennel-spawn) must
    ...
    // B: EXECUTE on the lib dirs too -> runs. This is the model kennel-spawn builds.
