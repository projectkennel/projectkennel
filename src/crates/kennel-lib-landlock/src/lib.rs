...
//! `kennel-lib-bpf`'s `bpf()`: `sys_create_ruleset` and `sys_add_rule` take typed
...
//! `kennel-lib-binder`): the hand-rolled Landlock ABI is a kernel boundary, parallel in structure to
//! `kennel-lib-bpf`. Split out of `kennel-lib-syscall` so that crate carries a smaller unsafe surface
//! (CODING-STANDARDS §4); re-exported there as `kennel_lib_syscall::landlock` for unchanged callers.
#![allow(unsafe_code)]
...
/// `kennel-lib-bpf`'s `bpf()`; because its only inputs are an `Option<&RulesetAttr>`
...
/// `CAP_SYS_ADMIN`. Inlined here (rather than depending on `kennel-lib-syscall`) so this crate stays
/// standalone, parallel to `kennel-lib-bpf`/`kennel-lib-binder`.
