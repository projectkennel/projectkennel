...
//! through `std` and nix's safe API. They were split out of `kennel-lib-syscall` (the one crate
//! permitted `unsafe`) so that crate carries *only* genuinely-unsafe code and stays reviewable in
//! one sitting (CODING-STANDARDS §4). `kennel-lib-syscall` re-exports these modules, so callers may
//! continue to reach them as `kennel_lib_syscall::{path, unistd, netlink, handshake}` unchanged.
//!
