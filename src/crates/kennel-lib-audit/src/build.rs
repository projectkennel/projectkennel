        ...
        match kind {
            SinkKind::File => {
                match FileSink::new(
                    ...
                    Err(e) => eprintln!("kennel-lib-audit: file sink unavailable: {e}"),
                }
            }
            ...
            SinkKind::Syslog => {
                ...
                match SyslogSink::new(PathBuf::from("/dev/log"), facility) {
                    Ok(sink) => push_buffered(&mut sinks, Box::new(sink)),
                    Err(e) => eprintln!("kennel-lib-audit: syslog sink unavailable: {e}"),
                }
            }
...
fn push_journald(_sinks: &mut Vec<Box<dyn Sink>>) {
    eprintln!("kennel-lib-audit: journald sink requested but built without audit-journald");
}
