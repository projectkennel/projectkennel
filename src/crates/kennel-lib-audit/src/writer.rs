    ...
    fn report_drop(&self, err: &SinkError, failed_index: usize) {
        let drop = Record {
            ...
            fields: vec![
                ...
                (
                    "reason",
                    Rendered::Str(kennel_lib_text::sanitise_for_audit(&err.message)),
                ),
            ],
        };
        ...
        if !delivered {
            eprintln!(
                "kennel-lib-audit: sink {} failed and no other sink accepted the drop record: {}",
...
fn sanitise(s: &str, changed: &mut bool) -> String {
    let clean = kennel_lib_text::sanitise_for_audit(s);
    if clean != s {
        *changed = true;
    }
    clean
