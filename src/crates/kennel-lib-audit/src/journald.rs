...
//! `SYSLOG_IDENTIFIER` is `kennel-lib-audit`. Submission goes through
//! `kennel_lib_syscall::journal::sendv` (the `sd_journal_sendv` FFI — the one crate
...
impl Sink for JournaldSink {
    ...
    fn write(&self, record: &Record) -> Result<(), SinkError> {
        ...
        fields.push("SYSLOG_IDENTIFIER=kennel-lib-audit".to_owned());
        ...
        kennel_lib_syscall::journal::sendv(&fields).map_err(|e| SinkError {
            sink: "journald",
