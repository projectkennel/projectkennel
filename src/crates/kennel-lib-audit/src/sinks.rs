...
fn gzip_file(path: &Path) {
        ...
        .status()
    {
        ...
        Ok(status) => eprintln!(
            "kennel-lib-audit: gzip of {} exited {}; left uncompressed",
            path.display(),
            status.code().unwrap_or(-1)
        ),
        Err(e) => eprintln!(
            "kennel-lib-audit: gzip of {} could not run ({e}); left uncompressed",
            path.display()
        ),
    }
}
...
impl SyslogSink {
    ...
    fn format(&self, record: &Record) -> String {
        ...
        let mut out = format!(
            "<{pri}>1 {ts} {host} kennel-lib-audit {procid} {msgid} {sd} {msg}",
            procid = self.procid,
...
mod tests {
    ...
    fn file_sink_rotates_and_retains() {
        let dir = std::env::temp_dir().join(format!("kennel-lib-audit-rot-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn file_sink_compresses_old_rotations() {
        ...
        let dir = std::env::temp_dir().join(format!("kennel-lib-audit-gz-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn file_sink_rejects_oversize() {
        let dir = std::env::temp_dir().join(format!("kennel-lib-audit-big-{}", std::process::id()));
        let sink = FileSink::new(dir.clone(), None, None, None).expect("create");
    ...
    fn syslog_format_is_rfc5424_shaped() {
        ...
        assert!(msg.starts_with("<12>1 2026-05-25T12:34:56.789012Z workstation kennel-lib-audit "));
