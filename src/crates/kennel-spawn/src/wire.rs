...
//! root-owned `kennel-init` as the kennel's uid-0 PID 1 (`kennel-init-and-uid0`).
//! `kenneld` (the operator) builds the [`Plan`]; it must reach `kennel-init` as bytes.
//! The privhelper forwards the blob opaquely; **`kennel-init` (root) decodes it**, so
