...
//! `kennel-bin-ssh-reorigin` binary: the thin IO shell around the re-origination core.
//!
//! All decision logic lives in the library (`kennel_bin_ssh_reorigin`), which is pure and
...
fn main() -> ExitCode {
    ...
    match run(&args) {
        ...
        Err(e) => {
            eprintln!("kennel-bin-ssh-reorigin: denied: {e}");
            ExitCode::from(1)
        }
    }
}
...
fn run(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "kennel-bin-ssh-reorigin: re-originating to {dest} (key {key}); synthetic {ktype} authenticated",
        dest = req.dest,
        key = req.key,
        ktype = auth.key_type,
    );
