//! `kennel-netproxy`: the per-kennel egress **dial delegate**.
//!
...
//!
//! All the logic is in the library (`kennel_netproxy::conduit`); `main` binds the socket and serves.
...
fn main() -> ExitCode {
    let Some(path) = std::env::args_os().nth(1) else {
        eprintln!("usage: kennel-netproxy <command-socket-path>");
...
fn run(sock: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    kennel_netproxy::conduit::serve_conduit(&listener);
    Ok(())
}
