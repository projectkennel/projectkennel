//! `kennel-netproxy`: the per-kennel egress **dial delegate** (a glorified `netcat(1)`).
//!
//! # What this is
//!
...
//! speaks SOCKS to `kennel-netshim` inside its own net-ns, which forwards each request as a
