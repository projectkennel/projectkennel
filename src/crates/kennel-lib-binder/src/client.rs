...
impl Connection {
    ...
    /// descriptor** in one reply (the `kennel-bin-init` `GET_SANDBOX_PLAN` pull: the
    /// supervision-half bytes plus the controlling-pty fd — `07-2` §7.2.3). Sets
    ...
    /// (the `kennel-bin-init` `GET_SANDBOX_PLAN` reply: supervision bytes + the pty fd). Then
