...
//! # Invocation
//!
//! `kennel-afunix-shim <binder-device> <shim-path>=<service-name> [...]`, spawned by
...
use kennel_binder::service::verb;
...
fn main() -> ExitCode {
    ...
    let Some(device) = args.next() else {
        eprintln!("kennel-afunix-shim: usage: <binder-device> <shim-path>=<service> ...");
        return ExitCode::FAILURE;
    };
    let sockets: Vec<(String, String)> = args.filter_map(|a| split_pair(&a)).collect();
    if sockets.is_empty() {
        eprintln!("kennel-afunix-shim: no <shim-path>=<service> pairs given");
        return ExitCode::FAILURE;
    }
    ...
    for (shim, service) in sockets {
        ...
        handles.push(thread::spawn(move || {
            if let Err(e) = serve_socket(&device, &shim, &service) {
                eprintln!("kennel-afunix-shim: {shim} ({service}): {e}");
            }
        }));
    }
...
fn serve_socket(device: &str, shim: &str, service: &str) -> io::Result<()> {
    ...
    for incoming in listener.incoming() {
        ...
        match broker(device, service) {
            ...
            Err(e) => {
                eprintln!("kennel-afunix-shim: facade refused {service}: {e}");
