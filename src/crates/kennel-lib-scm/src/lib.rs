...
//! across an `execve`.
//!
//! A small unsafe-bearing crate (parallel to `kennel-lib-landlock`/`kennel-lib-bpf`/`kennel-lib-binder`), split
//! out of `kennel-lib-syscall` and re-exported by it, so a dumb delegate like `host-netproxy` can
...
/// (`07-2` §7.2.1): `kenneld` keeps one end and hands the other to the factory as its
/// stdin, sending the construction-half plus the `kennel-bin-init`/pty fds (`SCM_RIGHTS`) one
...
mod tests {
    ...
    fn tempfile() -> std::fs::File {
        let path = std::env::temp_dir().join(format!("kennel-lib-scm-test-{}", std::process::id()));
