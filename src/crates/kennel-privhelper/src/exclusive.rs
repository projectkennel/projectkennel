//! The exclusive host-bind over-mount (§2.7, T2.8) — a transient privileged op.
//!
//! While a kennel runs, an `fs.exclusive` path is shadowed on the **host** side by an opaque
//! sentinel `tmpfs`, so the operator and the workload cannot use the path concurrently — this
//! severs the live confused-deputy channel (a workload planting "run this" content the operator
//! then acts on). The kennel keeps the real inode through its own (already-constructed,
//! rec-private) view; the over-mount lands only in the operator's session mount namespace.
//!
//! **Overreach refusal (the load-bearing check).** Blind-mounting with the helper's privilege
//! over a path the operator does not **own** would be overreach, so `mount_exclusive` refuses any
//! path whose owner is not the caller's real uid. `kenneld`/the CLI check this too (early
//! feedback); this is the authoritative gate — the helper trusts only the kernel-stamped real uid.
...
/// The sentinel marker dropped into the over-mounted tmpfs, naming the lock for an operator who
/// finds a directory shadowed (and a recovery hint for a leaked lock).
...
pub fn mount_exclusive(host: &Path, owner_uid: u32) -> Result<(), String> {
    ...
    if let Err(e) = mount::remount_readonly(host) {
        eprintln!(
            "kennel-privhelper: sealing exclusive over-mount of {} read-only failed: {e}",
            host.display()
        );
    }
    Ok(())
...
#[must_use]
pub fn unmount_exclusive(host: &Path) -> ExitCode {
    if !host.join(SENTINEL).is_file() {
        eprintln!(
            "kennel-privhelper: refusing exclusive release: {} is not a kennel exclusive over-mount",
            host.display()
        );
    ...
    match mount::unmount_detach(host) {
        ...
        Err(e) => {
            eprintln!(
                "kennel-privhelper: releasing exclusive over-mount of {} failed: {e}",
                host.display()
            );
            ExitCode::from(3)
        }
