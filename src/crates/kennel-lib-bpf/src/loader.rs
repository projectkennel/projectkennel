...
mod root_tests {
    //! Run via `sudo -E cargo test -p kennel-lib-bpf --features e2e`. Compiles
    ...
    fn compile_uapi(name: &str) -> Vec<u8> {
        ...
        let tmp = std::env::temp_dir().join("kennel-lib-bpf-test");
    ...
    fn load_attach_and_enforce_connect4() {
        ...
        let cg = Path::new("/sys/fs/cgroup/kennel-lib-bpf-test");
    ...
    fn connect_allowed_when_map_populated() {
        ...
        let cg = Path::new("/sys/fs/cgroup/kennel-lib-bpf-test-allow");
        let mut denied = true;
        with_attached_connect4(cg, &loaded, || denied = connect_denied_in_cgroup(cg));
        assert!(
            !denied,
    ...
    fn drains_audit_event_on_connect() {
        ...
        let cg = Path::new("/sys/fs/cgroup/kennel-lib-bpf-test-audit");
        let _ = std::fs::create_dir(cg);
        let cgfd = std::fs::File::open(cg).expect("open cgroup");
        let spec = KENNEL_PROGRAMS
            .iter()
    ...
    fn shared_maps_drain_one_ringbuf_across_programs() {
        ...
        let cg = Path::new("/sys/fs/cgroup/kennel-lib-bpf-test-shared");
        let _ = std::fs::create_dir(cg);
        let cgfd = std::fs::File::open(cg).expect("open cgroup");
    ...
    fn detach_restores_connectivity() {
        ...
        let spec = KENNEL_PROGRAMS
            ...
            .find(|p| p.name == "connect4")
            .expect("connect4 spec");
        let cg = Path::new("/sys/fs/cgroup/kennel-lib-bpf-test-detach");
        let _ = std::fs::create_dir(cg);
        let cgfd = std::fs::File::open(cg).expect("open cgroup");
        loaded
            .attach(cgfd.as_fd(), spec.attach_type)
    ...
    fn pin_and_get_program() {
        ...
        let loaded = load_connect4();
        let pin = c"/sys/fs/bpf/kennel-lib-bpf-test-pin";
        let pin_path = Path::new("/sys/fs/bpf/kennel-lib-bpf-test-pin");
    ...
    /// producer (`kennel-lib-spawn::plan`) writes it.
    ...
    fn bind4_enforces_the_min_port_floor() {
        ...
        let cg = Path::new("/sys/fs/cgroup/kennel-lib-bpf-test-bindfloor");
        ...
        let cg = Path::new("/sys/fs/cgroup/kennel-lib-bpf-test-nofloor");
        let mut denied = true;
        with_attached_bind4(cg, &loaded, || {
