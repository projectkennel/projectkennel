//! `kennel-bin-init` — the kennel's uid-0 PID 1.
//!
//! # Role
//!
...
//!
//! `kennel-bin-init` is a pure supervisor. It does **not** mount, pivot, provision binderfs,
...
//!    `kennel-lib-spawn::wire::encode_supervision` bytes (a plain data reply). For an interactive
//!    run the controlling-pty return socket is NOT pulled here — the factory placed it at
//!    `kennel_lib_syscall::pty::PTY_RETURN_FD`, which this process inherited across the `fexecve`.
...
//! `kennel-lib-syscall`, every binder primitive in `kennel-lib-binder`.
...
use kennel_lib_syscall::spawn::{fork_drop_exec, fork_drop_exec_confined};

/// The in-view binder device the factory mounts (`07-1` §7.1): a fixed path, because the
/// pull model gives `kennel-bin-init` no argv to carry it.
...
///
/// `kennel-bin-init` is only ever `fexecve`d by the privhelper factory as PID 1 of a fresh,
    ...
    if first != ["0", "0", "1"] {
        return Err(io::Error::other(format!(
            "refusing to run outside a kennel user namespace: /proc/self/uid_map first \
             mapping is {first:?}, expected [\"0\", \"0\", \"1\"] — kennel-bin-init is launched \
...
///
/// The factory does not `fexecve` `kennel-bin-init` until kenneld has claimed the context-manager
...
fn spawn_all(
    ...
    let seal = || -> io::Result<()> {
        ...
        let ruleset = kennel_lib_spawn::build_ruleset(&sup.landlock_fs, &sup.landlock_net, true)?;
        ...
        match reaped {
            ...
            Reaped::Exited { pid, .. } => {
                ...
                if !may_restart(&mut state.starts, Instant::now()) {
                    eprintln!(
                        "kennel-bin-init: facade {} hit the restart limit ({} starts in {:?}); \
                         leaving it down",
                        facade.path.display(),
                        START_LIMIT_BURST,
...
mod tests {
    ...
    fn facade_argv_uses_the_path_as_argv0() {
        ...
        assert_eq!(
            ...
            b"/usr/libexec/kennel/facade-afunix-shim"
        );
    ...
    fn decode_of_an_encoded_supervision_recovers_the_program() {
        let sup = Supervision {
            ...
            seccomp_deny_action: kennel_lib_syscall::seccomp::Action::KillProcess,
            ulimits: Vec::new(),
            aux: Vec::new(),
            interactive: false,
            ttl_seconds: None,
