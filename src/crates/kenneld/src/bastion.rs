...
impl Bastion {
    ...
    pub fn deregister(&mut self, kennel: &str) -> io::Result<()> {
        ...
        // Rewrite authorized_keys only if the bastion dir exists (i.e. the daemon was
        // started). With no daemon, edge bookkeeping is all there is to do.
    ...
    fn start_if_needed(&mut self) -> io::Result<()> {
        ...
        let params = SshdParams {
            ...
            auth: AuthSource::File(self.config.authorized_keys()),
        };
        let config_path = self.config.config_file();
        std::fs::write(&config_path, sshd::sshd_config(&params))?;
        // An authorized_keys must exist before sshd reads it on the first connection.
        self.write_authorized_keys()?;
