...
pub struct BinderPrep {
    ...
    /// Whether the standing `dbus-broker` service kennel is enabled in the catalogue, so the
    /// `SVC_CONNECT(dbus-name)` locator may reply the mesh device path. When clear, the locator
    /// refuses and the kennel's D-Bus goes unserved (fail-closed; there is no other mediation
    /// home).
...
/// the in-view `listen_path` and relays it to the standing dbus-broker over the connector mesh)
/// and points the workload's `DBUS_*_BUS_ADDRESS` there. Mediation is the broker's job — kenneld
/// holds no bus leg and no filter table. Empty ([`DbusPrep::default`]) for a kennel with no
/// `[dbus]` grant.
#[derive(Debug, Default, Clone)]
pub struct DbusPrep {
...
/// One enabled bus's facade wiring (§7.7).
#[derive(Debug, Clone)]
...
fn bring_up<P: Privileged + Sync>(
    ...
    //     needs to bind them. The standing dbus-broker mediates over the mesh, beside the
...
/// `apply_socks5`'s view-bind + Landlock + loader grants; the facade relays to the standing
/// dbus-broker over the connector mesh (§7.7).
///
