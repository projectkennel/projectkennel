...
pub static TABLES: &[Table] = &[
    ...
    Table {
        ...
        fields: &[
            ...
            f("reason", Ty::Str, "Why this capability is offered (required)."),
            f("threats", Ty::Obj("threats"), "Threat tags."),
        ],
    },
    Table {
        name: "consumes_entry",
        title: "One `[[consumes]]` entry — a capability this kennel reaches over the mesh (§7.13.1).",
        fields: &[
            f("name", Ty::Str, "The capability's public identifier, resolved against the catalogue at runtime."),
            f("shape", Ty::Enum(&["af-unix", "dbus-name", "binder-connector"]), "The transport it expects; the broker refuses a mismatched shape (§7.13.4)."),
            f("at", Ty::Str, "Where the brokered connector is delivered, in this kennel's own view."),
            f("env", Ty::StrArray, "Environment variable(s) synthesised into this kennel to name the connector."),
            f("key", Ty::Str, "Optional private match token; must match the provider's."),
            f("required", Ty::Bool, "Whether the capability's absence fails kennel construction (default true)."),
            f("reason", Ty::Str, "Why this capability is consumed (required)."),
            f("threats", Ty::Obj("threats"), "Threat tags."),
        ],
    },
    Table {
