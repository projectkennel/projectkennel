...
Project Kennel provides the enforcement vocabulary the user level should have acquired as it grew into a runtime. The host vocabulary catalogued in §1.4 — mandatory access control, the LSM framework, systemd's syscall and address-family filters — is at bottom a single idea: a *reference monitor*. A mediator every access passes through, that the constrained code cannot bypass, cannot tamper with, and that is small enough to audit. The user level has no such monitor. Project Kennel is one, built in user space.
 
A monitor assembled only from the kernel's own access control would inherit its ceiling. DAC decides whether a uid may open a channel and then stops; SELinux decides whether a domain may reach a label and then stops; neither sees what crosses the channel it allowed. This is why the platform already grew `dbus-daemon`, polkit, and ssh forced commands — userspace mediators bolted onto individual protocols because kernel access control granted the channel and had nothing to say about its use. Project Kennel generalises that move into one monitor across every resource class: it mediates the content of an interaction, not merely whether the interaction can be reached. §4 develops this as the design's spine — the three reference-monitor properties, and the two ways the monitor reaches complete mediation — and §7 implements it resource class by resource class.

The architectural commitments map onto the host-level vocabulary, each at user-level workload granularity:
