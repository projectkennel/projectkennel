...
- `binder` — kenneld's context-manager logic over the `kennel-lib-binder` primitive (`#![forbid(unsafe_code)]`): node-0 acquisition (open `/proc/<init-host-pid>/root/dev/binderfs/binder` for the operator-owned instance, then `BINDER_SET_CONTEXT_MGR`), the per-instance non-blocking looper, the per-kennel and cross-instance service registries, the `org.projectkennel.*` reserved services (the `IAfUnix/default` facade and the `INet` crossing), the `GET_SANDBOX_PLAN` reply that hands `kennel-bin-init` its supervision-half `Plan`, and the `NOTIFY_*` lifecycle verbs gated on `sender_pid == init_host_pid && sender_euid == 0`. The full contract is `02-4-binder.md`; the network-over-binder half is `02-5-binder-net.md`.

The **kenneld↔delegate socketpair protocol** (`02-5-binder-net.md`) is how kenneld reaches the non-binder network delegates: `host-netproxy` (the CONNECT delegate) and `host-inetd` (the BIND/mirror delegate) are **not** binder participants. On an approved `CONNECT_INET` transaction the looper decides it under `[net.proxy]`, pins the vetted address, and dials the delegate inline (`crate::inet::dial_via_delegate`): it mints a per-connection conduit socketpair, hands one end to the delegate over the per-kennel command socket via `SCM_RIGHTS`, and replies on the same transaction with the kennel-facing end as a binder fd object. Because the dial is blocking, the `INet`/af-unix facade verbs run off the registry lock so a slow delegate occupies one looper, never the whole pool. The inbound `REGISTER_MIRROR` path is push-based instead: kenneld maps `port → callback handle` and pushes each accepted conduit later with `DELIVER_INET`.
