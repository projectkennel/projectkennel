...
- The binder kernel feature for the binder test path: `CONFIG_ANDROID_BINDERFS` + `CONFIG_ANDROID_BINDER_IPC` (`=y` or a loaded/auto-loadable `binder_linux` module) and the binder UAPI headers (`linux/android/{binder,binderfs}.h`), pinned in `BUILD-ENV.md`. The plain `cargo build` does not need binder (the `kennel-lib-binder` ABI crate compiles against vendored UAPI constants); the *test* path that mounts binderfs does (see the binder load/test matrix below).
...
- `libbpf`, `vmlinux.h`, or any CO-RE machinery (the programs compile against the UAPI and are loaded by `kennel-lib-bpf`'s own `bpf(2)` loader; ELF parsing uses the `object` crate).
...
2. **BPF compilation.** `kennel-lib-bpf`'s `build.rs` invokes clang against `bpf/*.bpf.c`, producing `*.bpf.o` files in `OUT_DIR`. Each `.bpf.c` includes `<linux/bpf.h>` (kernel UAPI) and `bpf/maps.h`; **no** `vmlinux.h`, no CO-RE relocations. The `.o` is embedded into the crate; map references are left as ELF relocations the loader resolves at load time.
3. **Rust compilation.** `cargo build --workspace` builds every crate. The workspace is twenty crates: `kennel-lib-syscall`, `kennel-lib-os` (the safe, no-`unsafe` OS helpers — path/uid-gid/netlink/handshake — split out of `kennel-lib-syscall` and re-exported by it so that crate carries only genuinely-unsafe code), `kennel-lib-landlock` (the hand-rolled Landlock ABI, split out of `kennel-lib-syscall` and re-exported), `kennel-lib-scm` (the `SCM_RIGHTS` fd-passing helper, likewise split out and re-exported), `kennel-lib-text`, `kennel-lib-policy`, `kennel-lib-bpf`, `kennel-lib-audit`, `kennel-lib-config`, `host-netproxy`, `host-inetd` (the host-side egress-dial / inbound-BIND delegates), `kennel-privhelper`, `kennel-lib-spawn`, `facade-afunix`, `facade-socks5`, `facade-client`, `facade-ssh` (the in-kennel ends of binder connectors), `kennel-lib-binder` (the unsafe binder ABI crate, parallel to `kennel-lib-bpf`), `kennel-bin-init` (the root-owned PID-1 supervisor binary, `07-2-kennel-bin-init.md`), and `kenneld` (which also produces the `kennel` CLI binary alongside the `kenneld` daemon, in its `src/bin/`). Order is computed by Cargo from each member's `[dependencies]` (there is no `[workspace.dependencies]` table — members pin their own external versions); the lower-layer crates (`kennel-lib-os`, `kennel-lib-landlock`, `kennel-lib-syscall`, `kennel-lib-text`) are built before higher layers (`kennel-lib-spawn`, `kennel-bin-init`, `kenneld`). The full per-crate decomposition lives in `03-crate-decomposition.md`.
4. **Binary stripping** (release only). `strip = "symbols"` in the release profile. A separate `[profile.release-with-debuginfo]` (inheriting `release`, with `strip = "none"`, `debug = "full"`) produces parallel debug-info binaries under `target/release-with-debuginfo/` for distributions that want a `.debug` package or for production crash symbolication.
...
| `embed-programs` (`kennel-lib-bpf`) | Compile `bpf/*.bpf.c` with clang at build time and embed the objects | Needed wherever the loader must actually attach programs. |
| `bpf-egress` (`kennel-privhelper`) | Privhelper built with the BPF egress path (pulls in `kennel-lib-bpf` with `embed-programs`) | Root e2e and the privileged egress install. |
...
Most integration tests run as the user with no special capabilities. A subset (in `kennel-lib-spawn`, `kennel-lib-bpf`, and `kennel-privhelper`) require root for namespace operations, cgroup creation, Landlock sealing on a real kernel, and BPF attach. These are gated behind the `e2e` feature and run in a dedicated CI job.

### Property and round-trip tests

The deterministic-input properties — resolver/validator invariants (`kennel-lib-policy`), sanitiser totality (`kennel-lib-text`), and the wire-format encode/decode round-trip (`kenneld::control`) — are asserted as unit tests beside the code, run by the standard `cargo test` invocation. The adversarial-input side is covered by the `kennel-fuzz` harness below, which carves a seed into many boundary lengths via `arbitrary`.
...
- `host-netproxy::protocol::detect`, `socks5::parse_greeting`, `socks5::parse_request`, `http::parse_request` — the network front-door parsers.
- `host-netproxy::config::from_toml_str` — the proxy config reader (over `String::from_utf8_lossy` of the bytes).
...
- `kennel-lib-binder::proto::parse` / `TransactionData::from_bytes` / `flat_binder_object_fd_value` — the binder driver-return (`BC`/`BR`) command stream and the sender-controlled transaction payload the kernel fills into the read buffer (`02-4-binder.md`).
- `kennel-lib-spawn::wire` — the `Plan` decoder (`decode_plan`, `decode_construction`, `decode_supervision`). The same flat encoding crosses two privilege boundaries on input the operator built (the privhelper `ConstructKennel` op payload and the `kennel-bin-init` `GET_SANDBOX_PLAN` `BC_REPLY` payload, `07-2-kennel-bin-init.md`); the decoder is bounded by construction.
- `kennel-lib-policy::verify_settled` — the signed settled-policy reader (the TOML parse and schema-version gate run on the untrusted bytes before the signature check).
...
CI runs a dedicated job that, for each kernel version in the matrix (see `02-7-bpf-abi.md`), boots a VM (or uses a multi-kernel CI service), copies the built `.bpf.o` files in, and attempts `bpftool prog load` for each program. Failure on any matrix entry blocks merge — this is the BPF equivalent of a compile error.
...
**Roadmap CI additions** (the subsystems are built; the CI coverage is what is owed): root tests for the per-kennel net-ns path (`CLONE_NEWNET`, the four network modes, the loopback mirror, the host-side egress + inbound-BIND legs) — these run today via the `src/tools/policy-e2e.sh` suite against the installed service, and are owed as a CI job once the privileged e2e runner lands (it needs elevated capabilities; see the privileged-runner item above). The `kennel-facade` SOCKS5/HTTP front-end parser (`socks5::protocol::detect`, `socks5::http::parse_request`) and the `kennel-lib-binder` `BC`/`BR` decoder fuzz targets are *already* wired into the `fuzz` job (they are built), not roadmap.
...
`src/tools/unprivileged-e2e.sh` drives the full vertical of the privhelper *factory* construction model (`07-2-kennel-bin-init.md`, `02-4-binder.md`): an unprivileged, identity-mapped user namespace constructs the kennel, the privhelper mounts the per-instance binderfs and chowns the device, kenneld claims node 0 and serves the registry, `kennel-bin-init` pulls its `GET_SANDBOX_PLAN` over the bus, and the brokered `org.projectkennel.IAfUnix/default` facade returns a connected fd. The spawn itself runs unprivileged; the script's `sudo` steps are the deployment prerequisites (reversible and local):

- `sudo setcap cap_setuid,cap_setgid,cap_setfcap,cap_sys_admin=ep` on the privhelper. `cap_setuid` is the factory writing the `0 0 1` host-root line of the kennel's identity uid_map; **`cap_setfcap` is *also* required since Linux 5.12 to map host uid 0 into a new user namespace** (otherwise the `0 0 1` write is `EPERM` even for a privileged process). These join the privhelper's existing `cap_setgid` / `cap_sys_admin` (`07-paths.md`); the `cap_net_admin`/`cap_bpf`/`cap_perfmon` for loopback and egress live in the sub-helpers (`kennel-privhelper-net`/`-bpf`), not the factory. This is the deployed `setcap` line, not a test-only artefact.
...
The script builds the participants it needs (`kennel-bin-init`, the facade/proxy binaries, the test binary) and is the as-built proof for the binder construction vertical. Its privileged variant shares the `e2e` runner; the owed privileged CI runner (below) is where it runs in CI.
...
Audit events are JSON Lines: one well-formed JSON object per line, every string value escaped through `kennel-lib-text` so no raw control bytes reach the sink. `host-netproxy`'s audit module (and the daemons that write their own events) own the sink — a file under the kennel's state dir, or an fd kenneld passed. The test discipline is a render-and-reparse round trip: emit every record type, then parse each line back and compare against the canonical event, which catches escaping and field-mapping drift cheaply.

---

## Reproducible builds

The build-side machinery is built; the CI double-build-and-compare job is owed (it needs the pinned release image). `src/tools/reproducible-build.sh` pins `SOURCE_DATE_EPOCH` to the HEAD commit time and remaps the three host-specific path roots (workspace, cargo home, rustup home) via `--remap-path-prefix`; the `[profile.release]` `codegen-units = 1` removes parallel-codegen non-determinism; and `[profile.release-with-debuginfo]` builds the debug-info variant reproducibly. The remaining owed piece is the CI job that runs the build twice on two runners and compares hashes.

Release builds are designed to run twice on two different CI runners. The build process:

1. Pin `SOURCE_DATE_EPOCH` to the commit timestamp (`src/tools/reproducible-build.sh`).
2. Build with the pinned toolchain and pinned clang.
3. Hash every output artefact (`kennel`, `kenneld`, `kennel-privhelper`, each `.bpf.o`, the strip'd binaries, the debug-info packages).
