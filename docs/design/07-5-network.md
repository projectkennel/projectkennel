...
Exposing that listener to the host is the **inbound mirror on `org.projectkennel.INet/default`**, and it is the one place the binder data direction runs **into** the kennel. That is deliberate and follows the *inbound* direction: an external connection originates outside the kennel, so the leg that carries it inward is naturally **kenneld→kennel** — the mirror image of egress, where the kennel originates and so the kennel pulls outward. The facade **registers a mirror node and sleeps**; kenneld **pushes** each accepted conduit to it. The conduit it carries is still kenneld's own socketpair end, so nothing crosses *into* the TCB; what is new — the callback node the facade hands up — is bounded by three guards.

> **Build state.** This push model is the chosen inbound design. The current build is the earlier **pull** model (`facade-client` polls `BIND_INET`); the push delta — the `REGISTER_MIRROR`/`DELIVER_INET` verbs, the death-notify lifecycle, the one-way push with the per-port queue as a bounce buffer, and the facade's blocking server loop — is sequenced as a `kennel-lib-binder`/kenneld/`facade-client` change.

- **The host-side listener is a delegate (`host-inetd`).** A per-kennel host-side leg, the reverse of the `CONNECT` dialer (`host-netproxy`), binds each policy-mirrored `ip:port` on the host `lo` alias at the kennel's own IP, listens, and accepts. The mirror set is the policy's (`[net.bind].allowed_ports` plus single-port `[[net.bpf.bind.allow]]` rules); kenneld registers them with the delegate eagerly at bring-up. The bind decision is the cgroup `[[net.bpf.bind]]` ACL's, already enforced at the workload's `bind()`; the workload never decides what is exposed, and kenneld makes **no further policy decision** on the inbound path — if a connection reached the host-side socket, that is all that matters.
- **The delegate splices locally and pushes the conduit's kennel end.** On an accepted connection `host-inetd` mints a socketpair, splices the accepted connection to the host end itself, and pushes the *kennel* end (plus the port) to kenneld over the owner-only control socket (`SCM_RIGHTS`). kenneld is a pure fd router for inbound — it runs no per-connection splice and touches no payload byte (the same split as `host-netproxy`).
- **The in-kennel facade (`facade-client`) registers a mirror node and blocks.** At bring-up, for each mirrored port, it sends `REGISTER_MIRROR(port)` to node 0 passing its own binder node, then enters a binder server loop and **sleeps in the kernel** (`BINDER_WRITE_READ` blocks) — zero CPU, no poll, no looper held. For each accepted conduit kenneld pushes a **one-way `DELIVER_INET`** transaction to the registered node carrying the conduit fd; the kernel wakes the facade, which `connect()`s the workload's native listener at `<kennel-ip>:<port>` and splices. Delivery is push-immediate — no poll latency — and an idle mirror costs nothing.
- **Three guards keep the push bounded.** (1) *Lifecycle*: kenneld requests a **death notification** on each registered node, so a `BR_DEAD_BINDER` on kennel teardown drops the ref and stops pushing — kenneld never holds or transacts to a stale node. (2) *Backpressure*: `DELIVER_INET` is **one-way**, so kenneld never blocks a thread waiting on the facade; if the push bounces (the facade's binder buffer is full), kenneld **re-queues the conduit** in the per-port queue and retries on the next accept — the queue survives as the *bounce buffer*, never the primary path, and it is byte-bounded as today. (3) *Registration is port-gated, not caller-gated*: the facade and the workload share the persona uid, so kenneld cannot tell them apart by `sender_euid`; it accepts `REGISTER_MIRROR` only for a port in the policy mirror set. A workload that registers its own mirrored port merely receives its own inbound conduits — harmless (per-kennel binderfs, its own port), and exactly what the bind ACL already authorised.

Bytes flow `external → accepted connection → socketpair → facade-client → kennel-side service`; the delegate mints the socketpair so kenneld stays out of the data path, and the only **fd** crossing into the kennel is a benign socketpair end — no host listener fd, no accepted-connection fd ever crosses the boundary. The fd-passing trust verdict is unchanged: nothing crosses *into* the TCB. The operator sees the port at the kennel's own IP on host `lo`, mapping straight back to the kennel.

> **Push, not poll — and why the reversal is sound.** The rejected alternative is *pull*: the facade polls `BIND_INET` and kenneld replies with a pending conduit or `AGAIN`. Pull was the earlier build for two reasons — to keep kenneld a pure responder (no daemon-pushes-into-the-kennel surface), and because a *parked* `BIND_INET` would hold a binder looper hostage (a binder **reply is thread-bound**: the receiving thread must send the reply, so it cannot wait for an external connection). But pull pays for that with a geometric `AGAIN` re-arm (50 ms→1 s per port): idle mirrors wake a looper ~1×/s **forever** (CPU burn that scales with ports × kennels) and an inbound connection waits up to ~1 s for the next poll (delivery latency). Push removes both — the facade sleeps in the kernel and kenneld delivers on accept — and it **sidesteps the thread-bound-reply blocker** because `DELIVER_INET` is a *fresh transaction*, not a reply to a parked recv. It reverses the no-callback-node property **knowingly**: for *inbound*, kenneld→kennel is the logical data direction; the conduit fd stays out-of-TCB (the [fd-passing verdict, `02-4`](../architecture/02-4-binder.md) is intact); and the three guards bound the new node-ref surface. Egress keeps its pull — there the kennel originates, so the kennel pulls outward — so the two directions are symmetric in *data flow*, not in *who initiates the binder transaction*. Intra-kennel loopback connects (`facade-client` → the workload) pass the connect ACL via an any-port `/32` on the kennel's own loopback seeded by `stamp_proxy` — the kennel's own loopback is one trust domain in one net-ns.
