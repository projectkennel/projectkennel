...
Exposing that listener to the host is the **`BIND_INET` verb** on `org.projectkennel.INet/default`. It is **pull-based, the exact symmetric of `CONNECT`** — the in-kennel facade pulls inbound conduits the same way it pulls outbound ones, so the direction does *not* reverse and no daemon-initiated transaction (callback node) into the kennel is needed:
...
- **The in-kennel facade (`facade-client`) pulls each conduit.** For each mirrored port it transacts `BIND_INET` to node 0; kenneld replies with the next pending kennel-end fd, or `AGAIN` when none is waiting (the facade re-arms with a short backoff). The `BIND_INET` handler **never parks a binder looper** — the unbounded wait for an external connection lives in kenneld's per-port reader thread, off the binder pool, so eagerly mirrored idle ports cannot exhaust `POOL_MAX_THREADS`. On a delivered conduit the facade connects the workload's native listener at `<kennel-ip>:<port>` and splices.

Bytes flow `external → accepted connection → socketpair → facade-client → kennel-side service`; the delegate mints the socketpair so kenneld stays out of the data path, and the only fd crossing into the kennel is a benign socketpair end — the same conduit shape as `CONNECT`, pulled on the kennel's initiative. No host listener fd and no accepted-connection fd ever crosses the boundary. The operator sees the port at the kennel's own IP on host `lo`, mapping straight back to the kennel.

> **Pull-based, not the push/callback model.** An earlier draft had `facade-socks5` register a *callback node* and kenneld *push* inbound connections into it (the data direction reversing on every leg). The built design is simpler and avoids a daemon-pushes-fd-into-the-kennel surface: a dedicated `facade-client` *pulls* via `BIND_INET` exactly as `facade-socks5` pulls via `CONNECT_INET`. Eager mirroring (the host port is bound at bring-up, whether or not the workload is listening yet) plus the bounded-poll `AGAIN` re-arm keep kenneld a stateless fd router and the binder pool unblocked. Intra-kennel loopback connects (`facade-client` → the workload) pass the connect ACL via an any-port `/32` on the kennel's own loopback seeded by `stamp_proxy` — the kennel's own loopback is one trust domain in one net-ns.
