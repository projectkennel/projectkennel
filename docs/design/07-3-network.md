...
- **Composes with loopback isolation** (§7.3.6). The kennel's `127.42.x.1` is where the shim listens. The kennel cannot reach the user's `127.0.0.1` services — they are in a different network namespace — except by naming them as host services the proxy delegate dials on its behalf.
...
- `cgroup BPF bind`: gate every `bind()` against `[[net.bpf.bind]]` and report each *allowed* bind to kenneld to drive the host-side mirror (§7.3.7).
...
The SOCKS5 wire format is preserved end-to-end so the workload sees no change — it lives in `kennel-netshim` (§7.3.6), with `kennel-netproxy` reduced to the dial-and-vet delegate. SOCKS5 specifically because:
...
# computed from the kennel's tag and ctx — §7.3.2). There is no longer a
...
# reported to kenneld to drive the host-side mirror (§7.3.7).
[[net.bpf.bind]]
families = ["v4", "v6"]
...
# Bind handling — see §7.3.7. The bind allow/deny gate lives in [[net.bpf.bind]] above;
