...
- **The marker.** `.trust-manifest.json` at the root of each writable workspace pins the SHA-256 of the known execution triggers and lists untrusted-path globs (schema: `docs/schemas/trust-manifest-v1.json`; types/generation in `kennel-lib-manifest`). It is JSON so host IDEs validate it natively against the published schema.
...
- **Verification + re-pin (host).** Cooperating host tooling refuses to execute a trigger whose on-disk hash diverged from its pin. After legitimate edits the operator runs `kennel review <policy>` (host-side): it re-hashes the triggers, shows a diff of modified / removed / new ones, and on approval re-pins the manifest — the human sign-off the masked agent cannot perform.

Honesty: the masking is structural and complete (the agent provably cannot read or forge the manifest); it is best-effort only in *which* triggers it enumerates and in relying on host tooling to honour the marker — it informs the host, it does not enforce on it. See T2.8 for the residuals.
