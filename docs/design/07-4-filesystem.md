...
**`fs.home.sanitise`** constructs a sanitised copy of a host configuration file at kennel-lib-spawn time and bind-mounts the sanitised copy into the shim at the path the agent expects. Useful for `~/.gitconfig`, `~/.npmrc`, and similar config files where the agent needs the file to operate but specific keys (credential helpers, embedded tokens, URL rewrites) must not be visible. Project Kennel reads the real file, applies the `strip` patterns to remove matching keys, writes the result to a per-user staging location, and bind-mounts that into the shim at the path the agent expects.
...
### The masked workspace manifest (`.trust-manifest.json`)

The same over-mount mechanism `fs.scrub` uses for credential files closes **T2.8 (cross-context persistence via workspace triggers)** from the *host* side. A confined workload with `fs.write` to a project can plant a deferred trigger — a `Makefile` recipe, a `.git/hooks/` script, an IDE task — that fires later in the user's *unconfined* shell. The defence is a host-readable trust marker the confined workload cannot forge:

- **The marker.** `.trust-manifest.json` at the root of each writable workspace pins the SHA-256 of the known execution triggers and lists untrusted-path globs (schema: `docs/schemas/trust-manifest-v2.json`; types/generation in `kennel-lib-manifest`). It is JSON so host IDEs validate it natively against the published schema.
- **Generation (host, pre-flight).** `kennel run` ensures the manifest at each writable root *before* contacting kenneld — it reads `fs.write` from the settled policy, expands `~`/`$HOME` to the operator's real home (the same resolution that yields the bind *source*), and writes a baseline if absent. An existing manifest is left untouched; refreshing pins is the explicit `kennel review`, never an implicit `run` side effect (else it would launder an agent's edits). The generator is the CLI; kenneld is not involved and never links the JSON parser.
- **The mask (the structural guarantee).** The manifest lives inside a writable bind, so its host inode is otherwise reachable. The spawn view over-mounts an **empty, read-only file** at the manifest path inside every writable bind (after the binds, before `pivot_root`). Inside the kennel the manifest is therefore present-but-empty: the workload can neither read the pins nor write them. The host inode underneath is untouched — the host IDE reads the real manifest. This is *not* a Landlock deny: Landlock cannot make a file in a granted directory return ENOENT, and cannot block `stat`/`read` while the parent is readable. The view over-mount is the only mechanism that delivers the no-read property, and it is the `fs.scrub` mechanism applied to one well-known path. Gated by `[trust].manifest` (default on; §02-2).
- **The pinned bytes (content store).** Beside the manifest, `.trust-manifest.d/<sha256>` holds the pinned *bytes* of each content trigger (content-addressed, operator-owned, masked alongside the manifest). The manifest is the index; the store is what makes *show-the-diff* and *restore* possible, not just *detect*. The trigger set is an additive **catalogue** (`triggers.catalog`, vendor → `/etc/kennel` → `~/.config`) — operator-tunable and legible, with no hidden compiled default — plus escaping symlinks (a planted link pointing outside the writable boundary is itself a trigger class).
- **The live tripwire (`[trust].on_change`).** While the kennel runs, kenneld watches the pinned triggers and trigger directories on the host with an unprivileged `inotify` (operator context, notify-only) and acts on a mutation through the cgroup it already owns: `warn` (audit `fs.mutation`), `freeze` (suspend the workload for the operator), or `kill`. This is the one limb that reacts *live on the workload* rather than merely informing the host; best-effort (inotify can miss a race), with the teardown review as the authoritative backstop.
- **Verification + re-pin / restore (host).** Cooperating host tooling refuses to execute a trigger whose on-disk hash diverged from its pin. After legitimate edits the operator runs `kennel review <policy>`: it re-hashes the triggers, shows a *unified diff* (the live file against its pinned blob) of modified / removed / new ones, and on approval re-pins — the human sign-off the masked agent cannot perform. `kennel review --revert` instead **restores** each divergent trigger from its blob (mode preserved) and removes a planted one, scoped to the trigger paths.
- **The exclusive bind (`[fs.write].exclusive`, opt-in).** A writable bind marked exclusive is shadowed host-side for the run (the factory over-mounts an opaque sentinel on the source), so the operator and the workload cannot use the path concurrently — severing the *live* confused-deputy channel the enumerated manifest cannot cover. Ownership-gated (the privhelper refuses a blind mount over a path the operator does not own); released at teardown, `kennel release` for crash recovery. See §7.4.x / the T2.8 mitigation.

Honesty: the masking and the pins are structural and complete (the agent provably cannot read or forge them); the catalogue is best-effort only in *which* triggers it enumerates. The live tripwire and `--revert` mean the control no longer merely *informs* the host — it halts the workload on a mutation and can restore the baseline — but a planted trigger that survives review is still the operator's to run (the post-teardown detonation), which the exclusive bind closes for the run where enabled. See T2.8 for the residuals.
