...
**Wayland.** A display server protocol replacing X11, with a per-client capability model stricter than X11's — which is why it (unlike X11) admits a mediated grant: the nested-compositor display service (§7.14), where the workload's display server is an upstream compositor constructed inside a GUI-service kennel rather than the host's. X11 itself is a non-goal (§7.8).
