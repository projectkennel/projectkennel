...
**Family 2 — Posture degradation (T12–T18, T26).** What the workload does to the user's host configuration and the artefacts it produces. T12 (host security control deactivation) is the Ona-incident class. T13 (security-degrading changes in produced code) is the daily case across AI agents. T14–T16 cover secrets in unintended locations, over-privileged provisioning, and suppression of failing security checks. T17 and T18 cover clipboard, screen capture, and input synthesis. T26 (numbered after T25) covers cross-context persistence: a workload planting a git hook or build/IDE trigger in its writable tree that later fires in the user's unconfined shell.
