...
**Mitigation in Project Kennel.** X11 is not directly grantable. The X11-isolated path (Xwayland-isolated or Xephyr-isolated) places the workload in a separate X server with its own clipboard. Wayland clipboard isolation depends on compositor support; documented residual. For the **terminal escape channel**, the interactive PTY broker (§7.9.5a) runs the workload's PTY output through a streaming `vte`-based filter at its single master-read point and drops OSC 52 (clipboard), OSC 9 / 777 (notifications), and the DCS/APC/PM/SOS device-control bands while passing benign sequences (title, hyperlink, palette, CSI, C0). It is on by default (`[tty].filter_terminal_escapes`) and enforced for every attach and reattach — no client can opt out. It is best-effort: the standard state machine bounds a terminal-specific desync, and it shuts the low-effort injection class, not a determined desync.
...
**Mitigation in Project Kennel.** The **masked workspace manifest** (`.trust-manifest.json`, §7.4 / `[trust].manifest`, default on). The CLI maintains a manifest at each writable root pinning the SHA-256 of the known execution triggers (`Makefile`, `package.json`, `.vscode/tasks.json`, `.git/hooks/*`, …) and listing untrusted-path globs. The kennel **masks** the manifest — the spawn view over-mounts an empty file at its path inside every writable bind — so the confined workload cannot read or forge it: it can rewrite a `Makefile`, but cannot re-pin the manifest to match. Cooperating host tooling (a VS Code / `JetBrains` extension reading the manifest against the published schema) detects a trigger whose on-disk hash diverged from its pin and refuses to run it / drops the workspace to Restricted Mode. After legitimate edits the operator re-pins with `kennel review` (host-side, the human sign-off the masked agent cannot perform). This is a host-tooling-cooperation control, not a kernel deny — it closes the **detonation** step (host tooling won't run a tampered trigger) rather than the *write* (the agent still needs to edit project files). It composes with the older partial `fs.deny` on `.git/hooks/**` etc., which a template may still apply where a hard write-exclusion is acceptable.

**Residuals.** Honest scope: (1) the manifest defends only **enumerated** triggers — a trigger created *after* generation, or one of a kind not in the known set, is unpinned until `kennel review` surfaces it (it shows as a `new, unpinned` entry); (2) it relies on **host tooling honouring** the manifest — a host tool that ignores it, or a user who runs `make` by hand, is unprotected (the manifest informs, it does not enforce on the host); (3) `core.hooksPath` redirection into the writable tree still relocates hooks, though the relocated files are themselves triggers a future generation can enumerate. The masking itself is structural and complete (the agent provably cannot read or write the manifest), and is best-effort only in *which* triggers it covers. `[trust].manifest = false` opts a workload out entirely.
