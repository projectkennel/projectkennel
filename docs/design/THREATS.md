...
**Residuals.** Explicit grants in user policy can re-expose specific services (e.g., a `dev-server` template granting `127.0.0.1:5432`). Those grants are threat-tagged and surfaced in the diff. **Host-network reconnaissance residual:** a kennel deliberately shares the host *network namespace* (egress is gated by the cgroup BPF + the proxy, not by net-ns isolation — design §7.3), so the workload can *read* the host's network state — interfaces, routes, the listening-socket table, and the LAN neighbour (ARP) table — via both `/proc/net/*` and `AF_NETLINK` (`RTM_GETLINK`/`sock_diag`). This is **reconnaissance only**: it reveals where lateral movement *could* go, but the egress filter still blocks the workload from actually connecting anywhere but its proxy, so the recon cannot be acted on from inside the kennel. Masking `/proc/net` alone would not close it (netlink is an independent vector, and restricting netlink breaks `getaddrinfo`'s `AI_ADDRCONFIG`); the complete fix is a per-kennel **network namespace**, which would also make the network-inspection tools show the kennel's own (isolated) stack — a deferred re-architecture of the §7.3 egress/loopback model (tracked in `08-as-built-notes.md` §8.1). Until then it is an accepted residual; the locked templates do not grant network-inspection tools, so casual surfacing requires the workload's own code (e.g. an interpreter opening a netlink socket). **SSH agent / signing-oracle residual:** the obvious "expose a per-kennel ssh-agent socket" design fails here — the ssh-agent wire protocol carries an opaque to-be-signed blob, not a destination, so any exposed agent (or a fingerprint-filtering broker in front of one) is a *destination-blind signing oracle*: a hostile workload signs an allowlisted key against an attacker-chosen host and authenticates wherever that key is reused. The §7.8 bastion closes this by never exposing an agent and binding the destination to the authenticated synthetic key. The narrower residual that remains is **per-use authorisation**: a non-interactive key, once granted, can be driven freely *for its granted destinations* (gated only by the key's own custody — a hardware/sk key forces a touch; `[ssh] allow_headless` governs whether a CI kennel may use a non-touch key at all). The same blind-oracle problem applies to a per-kennel `gpg-agent` and is not yet addressed.
