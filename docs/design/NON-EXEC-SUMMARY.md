...
* **The Privileged Helper (`kennel-privhelper`):** The only component that holds elevated privilege, which it carries via file capabilities (setuid root only as a no-xattr fallback). It executes solely to write identity maps and mount the cage. It is written in safe Rust and operates only *after* an ed25519 signature check against the trust store verifies the policy template. It reads no untrusted input.
