...
> uid 0 (host root mapped `0 0 1`), the binderfs is mounted by the root-owned **`kennel-init`
> (PID 1)** the privhelper `execve`s, and `kennel-init` opens the device while uid 0 then
> **chowns `/dev/binderfs/binder` to the workload uid**. The mechanics below stand; the
> *actor* is `kennel-init` under privhelper-driven construction, and "no privileged step" no
> longer holds (the `0 0 1` map needs `CAP_SETUID`, so the privhelper writes it). `kennel-init`
> is itself a binder *consumer* — the lifecycle control plane (§7.11.2). Reconciliation owed
> (`../architecture/BINDER-NET-INTEGRATION.md`).
