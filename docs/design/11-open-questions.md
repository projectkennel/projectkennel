...
**Per-kennel user IDs via subuid/subgid — resolved (rejected).** An alternative once considered: allocate each kennel its own uid from the `/etc/subuid` range and run the kennel's processes as that uid, for kernel-level uid-based isolation in addition to Project Kennel's policy-level isolation. The trade-offs were always significant. Pros:
...
**Resolution.** Project Kennel does **not** allocate subuid/subgid ranges — "there be dragons": delegated subordinate ranges drag in the file-ownership, process-management, and administrator-setup complexity above without buying isolation the mechanism-based model does not already provide. The identity map is instead a precise pair of lines — host root `0 0 1` plus the operator identity `<operator> <operator> 1` (one further line per granted gid) — written in a single `write(2)` by the privhelper. The host-root `0 0 1` line gives the kennel a **real, structurally-confined uid 0** (see §7.2): system surfaces — the view root, `/dev`, the read-only library binds, and the binderfs control/device nodes — are owned by a proper root. The privhelper's construction child, running as that uid 0, builds those surfaces and chowns the binder device to the operator before handing off to `kennel-init` (PID 1, also uid 0, a pure supervisor that builds nothing). Operator code never runs as userns-0 — the privhelper `execve`s the trusted root-owned `kennel-init` only after `pivot_root`, so it is trapped from its first instruction — so mapping host root in carries no escalation surface. The threat model's "same-uid is no longer the trust boundary" claim is honoured by the mechanism-based isolation; subuid would have been additional defence in depth at a complexity cost judged not worth paying.
