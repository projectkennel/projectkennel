...
**Resolution.** Project Kennel does **not** allocate subuid/subgid ranges — "there be dragons": delegated subordinate ranges drag in the file-ownership, process-management, and administrator-setup complexity above without buying isolation the mechanism-based model does not already provide. The identity map is instead a precise pair of lines — host root `0 0 1` plus the operator identity `<operator> <operator> 1` (one further line per granted gid) — written in a single `write(2)` by the privhelper. The host-root `0 0 1` line gives the kennel a **real, structurally-confined uid 0** for the construction phase (see §7.11): system surfaces — the view root, `/dev`, the read-only library binds, and the binderfs control/device nodes — are owned by a proper root, and `kennel-init` (PID 1, uid 0) can build them and chown the binder device to the operator. It is *not* a `0 0 N` range, and there is no single-extent rule; the only constraint is the single-write discipline. Operator code never runs as userns-0 (the privhelper `execve`s the trusted root-owned `kennel-init`, which is trapped post-`pivot_root`), so mapping host root in carries no escalation surface. The threat model's "same-uid is no longer the trust boundary" claim is honoured by the mechanism-based isolation; subuid would have been additional defence in depth at a complexity cost judged not worth paying.
