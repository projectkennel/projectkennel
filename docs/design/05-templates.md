...
Keys are Ed25519 in the **OpenSSH wire format**: public keys are `ssh-ed25519 <base64-blob> <comment>`; private keys are `-----BEGIN OPENSSH PRIVATE KEY-----` PEM envelopes (unencrypted). `kennel keygen <key-id>` generates a key pair via `ssh-keygen -t ed25519`. A legacy raw-base64 format (seed as `<key-id>.key`, public key as `<key-id>.pub`) is also accepted (auto-detected by content); `kennel keygen migrate` converts legacy pairs to OpenSSH format in place.
