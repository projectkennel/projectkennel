...
**`fs.home.sanitise`** constructs a sanitised copy of a host configuration file at kennel-spawn time and bind-mounts the sanitised copy into the shim at the path the agent expects. Useful for `~/.gitconfig`, `~/.npmrc`, and similar config files where the agent needs the file to operate but specific keys (credential helpers, embedded tokens, URL rewrites) must not be visible. The design reads the real file, applies the `strip` patterns to remove matching keys, writes the result to a tmpfs location under `/run/kennel/<kennel>/sanitised/`, and bind-mounts that location into the shim.

**Status: not yet built (roadmap).** The `[[fs.home.sanitise]]` policy fields parse and validate, and fold up the template chain, but they are dropped at translate (they are source-only, like the other informational fields) and there is no shim-construction step that reads, strips, and binds the sanitised copy. The design below is the intended behaviour, not current behaviour.

**`fs.scrub`** overlays a tmpfs over files within otherwise-granted directories that match a glob pattern. The canonical use case is hiding `.env`, `.env.*`, `terraform.tfstate`, `*.pem`, `*.key`, and similar credential-shaped files within the project tree. The agent can read the file but sees either an empty file (`mode = "empty"`, the default) or ENOENT (`mode = "enoent"`, stricter but breaks tools that test for file existence). The design iterates the granted directories at shim construction time, finds files matching the patterns, and overlays a per-file tmpfs at each match.

**Status: not yet built (roadmap).** The `[fs.scrub]` policy fields parse and fold, but they too are dropped at translate and no shim-construction step iterates the granted trees and overlays the matched files. The design below is the intended behaviour, not current behaviour.
