...
**Start → `starting`.** A `Start` request arrives over the control socket (the CLI passes the workload's stdio fds via `SCM_RIGHTS`). kenneld allocates a `<ctx>` byte, records the kennel in the registry, and begins bring-up: verify the settled policy (`02-2`), invoke the privhelper to add the loopback addresses and create the cgroup, write the proxy config and launch `host-netproxy`, then drive construction. kenneld splits the settled `Plan` into a construction half and a supervision half and invokes the privhelper *factory* (`ConstructKennel`, `02-6-ipc.md`): the factory clones the namespaces, writes the identity maps, builds the view, mounts the per-kennel binderfs instance and chowns the `binder` device to the operator, `pivot_root`s, and `fexecve`s `kennel-bin-init`. kenneld then acquires binder node 0 by opening `/proc/<init-host-pid>/root/dev/binderfs/binder` (the init host pid arrives from the factory over the construction socketpair) and serves `kennel-bin-init`'s `GET_SANDBOX_PLAN` pull (the supervision-half bytes plus the pty fd).
...
**Roadmap — proxy as a binder-coupled CONNECT delegate.** Under the per-kennel network-namespace redesign (`02-5-binder-net.md`, ROADMAP), `host-netproxy` stops being reached by a loopback TCP listener and becomes kenneld's host-net-ns **CONNECT delegate**, attached to a per-kennel `kenneld`↔delegate socketpair opened during construction. Its lifetime stays coupled to the kennel (one proxy per kennel, launched after binderfs is up, reaped at teardown); only the inbound half changes — it no longer binds a SOCKS5 listener. The new in-kennel `facade-netshim` (binder consumer, SOCKS5 inbound) is forked by the in-kennel reaper into the kennel's namespaces and torn down with them. The host-side spawn leg holds the per-kennel host-side mirror sockets (lifetime = kennel, host-side attribution) as kenneld's **BIND delegate**. Teardown ordering gains a privhelper `RemoveLoopbackAlias` (the mirror of the construction-time `AddLoopbackAlias`) to drop the kennel's `/28`+`/64` from the host `lo`; the in-kennel listeners and the `facade-netshim` go with the net/mount namespaces, and the delegate socketpairs close when kenneld and the delegates exit. (The kennel still shares the host network namespace today; the per-kennel net-ns, the four modes, the mirror, and these delegate lifecycles are all roadmap.)
