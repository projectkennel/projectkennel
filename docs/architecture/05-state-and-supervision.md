...
**Roadmap — proxy as a binder-coupled CONNECT delegate.** Under the per-kennel network-namespace redesign (`02-5-binder-net.md`, ROADMAP), `host-netproxy` stops being reached by a loopback TCP listener and becomes kenneld's host-net-ns **CONNECT delegate**, attached to a per-kennel `kenneld`↔delegate socketpair opened during construction. Its lifetime stays coupled to the kennel (one proxy per kennel, launched after binderfs is up, reaped at teardown); only the inbound half changes — it no longer binds a SOCKS5 listener. The new in-kennel `facade-socks5` (binder consumer, SOCKS5 inbound) is forked by the in-kennel reaper into the kennel's namespaces and torn down with them. The host-side spawn leg holds the per-kennel host-side mirror sockets (lifetime = kennel, host-side attribution) as kenneld's **BIND delegate**. Teardown ordering gains a privhelper `RemoveLoopbackAlias` (the mirror of the construction-time `AddLoopbackAlias`) to drop the kennel's `/28`+`/64` from the host `lo`; the in-kennel listeners and the `facade-socks5` go with the net/mount namespaces, and the delegate socketpairs close when kenneld and the delegates exit. (The kennel still shares the host network namespace today; the per-kennel net-ns, the four modes, the mirror, and these delegate lifecycles are all roadmap.)
