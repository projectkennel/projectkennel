...
| 15 | Grace-period reaping (60s) + `draining`/reclaim state machine + per-kennel daemon reference counting (`01-process-model.md`, `05-state-and-supervision.md`) | **Dropped (maintainer).** The *daemon* (kenneld) persists for the user session (socket-activated user service); per-kennel resources (proxy, loopback addresses, cgroup, view) tear down **immediately** when the workload exits. No grace window, no `draining`/reclaim, no named-kennel refcount — one `kennel run` is one kennel. | A grace window only mattered to avoid daemon churn; the daemon already persists, so it buys nothing. Simpler lifecycle, no concurrent-teardown races. | `kenneld/src/lib.rs` (`Kennel::stop` → immediate `teardown`) |
| 16 | Two proxy listeners (v4+v6) — single-listener limitation | **Built**: `Proxy::serve_all` over a listen set; kenneld passes both loopback addresses. | Dual-stack egress reaches the proxy over either family. | `kennel-netproxy/src/{server,config,main}.rs`, `kenneld/src/{proxy,lib}.rs` |
