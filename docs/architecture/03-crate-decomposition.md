...
| `kennel-lib-policy` | 1737 | — | **yes** | cli, compile, kenneld, spawn | basic-toml, ed25519-compact, object, serde |
...
**Totals.** 24 crates, **31335 SLOC** (excluding `#[cfg(test)]`). The runtime **TCB closure** (the first-party dependency graph of `kenneld` / `kennel-privhelper` / `kennel-bin-init`) is 16 crates, **19334 SLOC**; the remaining 8 crates (12001 SLOC) are outside it (the operator CLI and its deps, the in-kennel facades, and the out-of-TCB D-Bus mediation engine).
