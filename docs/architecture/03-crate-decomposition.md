...
|---|--:|:--:|:--:|---|---|
| `kenneld` | 5943 | — | **yes** | *(2 bins)* | basic-toml, serde |
...
**Totals.** 24 crates, **30291 SLOC** (excluding `#[cfg(test)]`). The runtime **TCB closure** (the first-party dependency graph of `kenneld` / `kennel-privhelper` / `kennel-bin-init`) is 16 crates, **18945 SLOC**; the remaining 8 crates (11346 SLOC) are outside it (the operator CLI and its deps, the in-kennel facades, and the out-of-TCB D-Bus mediation engine).
