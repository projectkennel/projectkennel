...
|---|--:|:--:|:--:|---|---|
| `kenneld` | 7067 | — | **yes** | *(2 bins)* | basic-toml, serde |
...
**Totals.** 24 crates, **32926 SLOC** (excluding `#[cfg(test)]`). The runtime **TCB closure** (the first-party dependency graph of `kenneld` / `kennel-privhelper` / `kennel-bin-init`) is 16 crates, **20572 SLOC**; the remaining 8 crates (12354 SLOC) are outside it (the operator CLI and its deps, the in-kennel facades, and the out-of-TCB D-Bus mediation engine).
