...
|---|--:|:--:|:--:|---|---|
| `kenneld` | 7296 | — | **yes** | *(2 bins)* | basic-toml, serde |
...
| `kennel-lib-control` | 719 | — | **yes** | cli, compile, kenneld | — |
...
**Totals.** 26 crates, **33756 SLOC** (excluding `#[cfg(test)]`). The runtime **TCB closure** (the first-party dependency graph of `kenneld` / `kennel-privhelper` / `kennel-bin-init`) is 16 crates, **21094 SLOC**; the remaining 10 crates (12662 SLOC) are outside it (the operator CLI and its deps, the in-kennel facades, and the out-of-TCB D-Bus mediation engine).
