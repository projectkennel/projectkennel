...
The workspace has **17 crates**: `kennel-lib-policy`, `kennel-lib-syscall`, `kennel-lib-os`, `kennel-lib-landlock`, `kennel-lib-bpf`, `kennel-lib-binder`, `kennel-lib-audit`, `kennel-lib-config`, `kennel-lib-spawn`, `host-netproxy`, `kennel-privhelper`, `kennel-bin-init`, `kenneld`, `facade-afunix-shim`, `kennel-lib-text`, `kennel-bin-ssh-reorigin`, and `kennel-socks-connect`. `kennel-lib-os` holds the **safe** OS primitives — path canonicalisation, uid/gid identity, per-kennel netlink address management, and the userns-map pipe handshake — split out of `kennel-lib-syscall` so that crate carries *only* genuinely-unsafe code and stays reviewable in one sitting (CODING-STANDARDS §4). `kennel-lib-landlock` is the hand-rolled Landlock ABI — the **fourth** `unsafe`-bearing crate (after `kennel-lib-syscall`, `kennel-lib-bpf`, `kennel-lib-binder`), the largest single unsafe module, likewise split out of `kennel-lib-syscall`. `kennel-lib-syscall` depends on both and re-exports them, so callers reach them as `kennel_lib_syscall::{path, unistd, netlink, handshake, landlock}` unchanged. `facade-afunix-shim` is the in-kennel half of the `org.projectkennel.IAfUnix/default` facade (`07-6-afunix.md`): the small proxy `kennel-bin-init` launches inside the view that brokers the workload's AF_UNIX connect through the binder gateway to kenneld. `kennel-lib-binder` is the third unsafe-bearing crate — the hand-rolled binder ioctl ABI (the per-kennel inter-namespace gateway, `02-4-binder.md`), parallel in every structural respect to `kennel-lib-bpf`; binder is load-bearing, so every kennel links it through kenneld and `kennel-bin-init`. `kennel-bin-init` is the root-owned PID-1 binary the privhelper `fexecve`s to construct and supervise each kennel (`07-2-kennel-bin-init.md`). `kennel-lib-audit` is a first-class crate — the unified audit writer (the canonical event, one sanitisation pass, per-class level filtering, and the `Sink` fan-out). `kennel-lib-config` is a first-class crate too — the layered deployment/user configuration (`system.toml` / `config.toml` cascades) that keeps install paths out of the binaries. The last two crates are standalone, std-only SSH helpers (`07-10-ssh.md` §7.10.4) that depend on no other Project Kennel crate by design — they must stay minimal and self-contained: `kennel-bin-ssh-reorigin` is the bastion's re-origination forced command, and `kennel-socks-connect` is the `ssh` `ProxyCommand` that SOCKS5s through the egress proxy to reach the bastion. The CLI and the control/wire IPC are folded rather than carved into their own crates: the control protocol lives in `kenneld::control`, the privhelper wire in `kennel-privhelper::wire`, and the `kennel` CLI is a binary inside `kenneld` (`src/bin/kennel.rs`). A wire protocol shared by exactly two binaries is a module in one of them, not a third crate, and the CLI and daemon ship from the same crate so their protocol cannot drift. The whole workspace is blocking, thread-per-connection; no async runtime is linked.
...
│   ├── crates/                      Rust workspace members (17)
│   │   ├── kennel-lib-syscall/          syscalls/namespaces/seccomp/FFI (unsafe); re-exports kennel-lib-os + kennel-lib-landlock
│   │   ├── kennel-lib-os/               safe OS helpers (path, uid/gid, netlink, handshake); re-exported by kennel-lib-syscall
│   │   ├── kennel-lib-landlock/         hand-rolled Landlock ABI (unsafe; 4th unsafe crate); re-exported by kennel-lib-syscall
...
│   │   ├── host-netproxy/         binary + lib: SOCKS5/HTTP egress proxy (blocking, thread-per-conn)
...
│   │   ├── kennel-bin-ssh-reorigin/     binary + lib: SSH re-origination forced command (std-only; §7.10.4)
...
```
  ...
  * kennel-lib-bpf, kennel-lib-binder and kennel-lib-syscall are the three unsafe-bearing crates;
  ...
  kennel-bin-ssh-reorigin (bin)   ← stands alone: std-only, no Project Kennel deps.
...
- **`kennel-lib-syscall` is the primary `unsafe`-bearing crate**, alongside `kennel-lib-bpf` (hand-rolled `bpf(2)` FFI) and `kennel-lib-binder` (hand-rolled binder `ioctl(2)` FFI). Every other crate carries `#![forbid(unsafe_code)]` per CODING-STANDARDS.md §4.
- **Binder is confined to two participants.** `kennel-lib-binder` (the binder ABI) is linked only by `kenneld` (node 0 / context manager) and `kennel-bin-init` (PID-1 lifecycle consumer pulling its plan over node 0); no other crate links it. The roadmap `facade-netshim` (below) becomes a third linker when the network crossing is built. The workload never links `kennel-lib-binder`.
...
- **Roadmap (per-kennel net-ns, `02-5-binder-net.md`):** when the network crossing is built, the proxy is no longer reached by a TCP loopback listener — it becomes a host-net-ns CONNECT delegate behind the `org.projectkennel.INet` node, dropping its SOCKS5 *server* and gaining a delegate-socketpair reader. The SOCKS5 *server* role moves into the roadmap `facade-netshim` crate (next item). Not built — the kennel still shares the host network namespace.

### `facade-netshim` (roadmap — not built)

- A **roadmap** crate (`02-5-binder-net.md`, `07-5-network.md`): the per-kennel network shim that runs inside the kennel net-ns when the network crossing is built. It holds the **fuzzed SOCKS5** server the workload egresses through, translating to `org.projectkennel.INet` `CONNECT` transactions over the binder bus (kenneld then dispatches to the `host-netproxy` host-net-ns delegate). It would become the **third** binder participant after kenneld and `kennel-bin-init`. Not built — there is no `facade-netshim` crate in the workspace today, and the kennel still shares the host network namespace.
