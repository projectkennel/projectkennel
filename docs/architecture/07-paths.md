...
- **No cross-user clobber** — the privhelper resolves the path from its own
  *real* uid (it is file-capped but runs for the calling user), never the wire, so it
...
| `kennel-privhelper` | `/usr/libexec/kennel/kennel-privhelper` | `install.sh` installs it with file capabilities `cap_setuid,cap_setgid,cap_setfcap,cap_sys_admin=ep`, with setuid root (mode `4755`, owner root) only as the no-xattr fallback for filesystems that cannot carry file caps. The privhelper is the kennel *constructor* (`../design/07-2-kennel-bin-init.md`): it clones the namespaces as the operator (so the userns is operator-owned), writes the identity map (host root `0 0 1` + the operator line + one line per granted gid) in a single `write(2)`, builds the root-owned surfaces, mounts binderfs, `pivot_root`s, and `fexecve`s `kennel-bin-init`. `cap_setuid` writes the `0 0 1` uid map (the kennel's real uid 0); `cap_setgid` writes the `gid_map` so a workload keeps a granted supplementary group (§7.4.8); `cap_setfcap` lets the single map `write(2)` land; `cap_sys_admin` is the kernel's `uid_map`-write gate for mapping host uid 0 and mounts (view, `/dev`, binderfs) and `pivot_root`s. The loopback and egress caps are **not** here — the rare host-context operations live in three single-purpose sub-helpers (below). Not on `PATH`; located by absolute path from kenneld. |
| `kennel-privhelper-net` | `/usr/libexec/kennel/kennel-privhelper-net` | File capability `cap_net_admin=ep`; the factory execs it on demand to add/delete the host-`lo` mirror loopback address (netlink). Not on `PATH`. |
| `kennel-privhelper-bpf` | `/usr/libexec/kennel/kennel-privhelper-bpf` | File capabilities `cap_bpf,cap_net_admin,cap_perfmon=ep`; the factory execs it on demand (`host` mode only) to attach the host-mode egress BPF to the kennel cgroup (`cap_perfmon` because the cgroup-sockaddr program reads kernel context under `kernel.unprivileged_bpf_disabled`). Not on `PATH`. |
| `kennel-privhelper-mounts` | `/usr/libexec/kennel/kennel-privhelper-mounts` | File capability `cap_sys_admin=ep`; the factory execs it on demand to over-mount the exclusive-bind sentinel. Not on `PATH`. |
...
- **Run policies** — the settled leaf the daemon enforces — verify against **system keys *or* the user's own `~/.config/kennel/keys`**. A user may run a policy signed with their own key: a leaf can only narrow *within* the template's re-asserted invariants and a kennel runs with the user's own authority, so trusting the user's own run-policy signature grants no escalation. The daemon searches the **vendor** dir (`/usr/lib/kennel/keys`) first, then the admin `trust_dir`, then the user's, **earlier dirs winning** a key-id clash — so neither an admin nor a user key can shadow the vendor maintainer key. (`kenneld` `TrustStoreLoader::from_trust_dirs`.)
- **Reserved-namespace authority is by provenance.** A key loaded from the **vendor** dir is the authority for the built-in `org.projectkennel.*` namespace (`07-13-service-catalog.md` §7.13.5): the daemon refuses to load a settled policy that *provides* an `org.projectkennel.*` name unless a vendor-provenance key signed it, and a host-declared `[[reserved]]` namespace (§7.13.5a) only by the key-ids that entry authorizes. An admin or user key — trusted to sign a run policy — therefore cannot claim the project's own (or an unauthorized host) namespace, closing the provider-name-spoofing channel. (`kenneld::catalogue::provide_authorized`; the catalogue's *membership* projection and the broker that resolves against it both ride this gate.)
...
- **`/usr/lib/kennel/keys/*.pub`** and **`/etc/kennel/keys/*.pub`** mode `0644`: public keys; world-readable is fine. The **project maintainer key** ships in the vendor dir (`/usr/lib/kennel/keys`, package-owned) — its vendor provenance is the `org.projectkennel.*` authority; the admin dir holds org/customer keys. Private keys are not in either tree.
- **`kennel-privhelper`** file caps `cap_setuid,cap_setgid,cap_setfcap,cap_sys_admin` (setuid-root the no-xattr fallback): a compromise of the calling process (kenneld) does not automatically gain privilege; the privhelper validates every request per `04-trust-boundaries.md` boundary 1.
