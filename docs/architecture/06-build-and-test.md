...
**Roadmap CI additions** (designed, not built — the network-namespace redesign of `02-5-binder-net.md` / `07-5-network.md` is not built; the kennel still shares the host network namespace): root tests for the per-kennel net-ns path (`CLONE_NEWNET`, the four network modes, the loopback mirror, the host-side leg, `AddLoopbackAlias`/`RemoveLoopbackAlias`); and a fuzz target for the `facade-socks5` SOCKS5 front-end once that crate exists. The `kennel-lib-binder` `BC`/`BR` decoder fuzz target is *already* wired into the `fuzz` job (it is built), not roadmap.
