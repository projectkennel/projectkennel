...
**Roadmap CI additions** (the subsystems are built; the CI coverage is what is owed): root tests for the per-kennel net-ns path (`CLONE_NEWNET`, the four network modes, the loopback mirror, the host-side egress + inbound-BIND legs) — these run today via the `src/tools/policy-e2e.sh` suite against the installed service, and are owed as a CI job once the privileged e2e runner lands (it needs elevated capabilities; see the privileged-runner item above); and a fuzz target for the `facade-socks5` SOCKS5/HTTP front-end parser (the crate exists; the harness entry into the `fuzz` workspace is the remaining wiring). The `kennel-lib-binder` `BC`/`BR` decoder fuzz target is *already* wired into the `fuzz` job (it is built), not roadmap.
...
3. Hash every output artefact (`kennel`, `kenneld`, `kennel-privhelper`, `host-netproxy`, each `.bpf.o`, the strip'd binaries, the debug-info packages).
