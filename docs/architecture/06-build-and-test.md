...
3. **Rust compilation.** `cargo build --workspace` builds every crate. The workspace is seventeen crates: `kennel-lib-syscall`, `kennel-lib-os` (the safe, no-`unsafe` OS helpers — path/uid-gid/netlink/handshake — split out of `kennel-lib-syscall` and re-exported by it so that crate carries only genuinely-unsafe code), `kennel-lib-landlock` (the hand-rolled Landlock ABI — the fourth `unsafe`-bearing crate, also split out of `kennel-lib-syscall` and re-exported), `kennel-lib-text`, `kennel-lib-policy`, `kennel-lib-bpf`, `kennel-lib-audit`, `kennel-lib-config`, `host-netproxy`, `kennel-privhelper`, `kennel-lib-spawn`, `kennel-bin-ssh-reorigin`, `kennel-socks-connect`, `facade-afunix-shim`, `kennel-lib-binder` (the unsafe binder ABI crate, parallel to `kennel-lib-bpf`), `kennel-bin-init` (the root-owned PID-1 supervisor binary, `07-2-kennel-bin-init.md`), and `kenneld` (which also produces the `kennel` CLI binary alongside the `kenneld` daemon, in its `src/bin/`). The roadmap `facade-netshim` crate (`02-5-binder-net.md`) is not yet present. Order is computed by Cargo from each member's `[dependencies]` (there is no `[workspace.dependencies]` table — members pin their own external versions); the lower-layer crates (`kennel-lib-os`, `kennel-lib-landlock`, `kennel-lib-syscall`, `kennel-lib-text`) are built before higher layers (`kennel-lib-spawn`, `kennel-bin-init`, `kenneld`). The full per-crate decomposition lives in `03-crate-decomposition.md`.
