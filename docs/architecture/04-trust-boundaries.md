...
**Privilege held.** The privhelper is installed **setuid-root** (mode 4755, owner root; `src/tools/install.sh`), so it runs with euid 0 and the full root capability set — of which it actually exercises `cap_setuid` + `cap_setgid` + `cap_setfcap` (+ `cap_sys_admin` + `cap_net_admin`). (A file-capability install pinning exactly that set is a documented alternative — `src/tools/unprivileged-e2e.sh` — but the default install is setuid.) `cap_setuid` and the `cap_setfcap` single-`write(2)` are what let it write the precise two-line identity map — `0 0 1` (host root mapped to the kennel's uid 0) plus `<operator> <operator> 1`, plus one line per granted gid — with **no subuid/subgid** and no `0 0 N` range. The privhelper is the kennel **factory**: it builds the namespaces, writes the identity map, and `fexecve`s the root-owned `kennel-bin-init` (`01-process-model.md`, `02-4-binder.md` §Privilege).
