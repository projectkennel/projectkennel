...
- **AF_UNIX socket shim as the user-level analogue of D-Bus policy and systemd socket constraints.** Per-kennel views of `$HOME` and `$XDG_RUNTIME_DIR` contain only the sockets policy explicitly grants. Abstract-namespace sockets are denied categorically.
- **D-Bus access via xdg-dbus-proxy.** Method-level filtering per kennel, with audit.
...
The kernel mechanisms are not new. Landlock, cgroup BPF, mount namespaces, PID namespaces, seccomp, AppArmor, `PR_SET_NO_NEW_PRIVS` — all upstream Linux for years. Project Kennel is the combination as a coherent user-level enforcement framework, with the declarative policy language and signed-template ecosystem that the host level has had for two decades.
