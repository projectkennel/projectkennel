...
| `kennel-lib-spawn` | 2190 | — | **yes** | bin-init, kenneld, privhelper | — |
...
**Totals.** 30 crates, **37499 SLOC** (excluding `#[cfg(test)]`). The runtime **TCB closure** (the first-party dependency graph of `kenneld` / `kennel-privhelper` / `kennel-bin-init`) is 16 crates, **22655 SLOC**; the remaining 14 crates (14844 SLOC) are outside it (the operator CLI and its deps, the in-kennel facades, and the out-of-TCB D-Bus mediation engine).
