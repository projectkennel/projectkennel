...
**Residuals.** Within the project tree (typically granted), the workload can still encounter `.env*` files, embedded credentials in test fixtures, `terraform.tfstate`, and similar in-band secrets. The `fs.scrub` policy provides default masking of these patterns within otherwise-granted directories. Determined recovery via indirect paths (`git show HEAD:.env` on a scrubbed file) is not prevented; documented as a known limitation. **Persona reconnaissance (accepted, not actionable):** a confined workload can learn the host's hostname (`uname -n` — the UTS namespace is not unshared by default; the opt-in `[identity].hostname` masks it) and the operator's username (a `[fs.cwd]` grant binds the real `/home/<user>/…` path). Accepted rather than closed: an agent-class workload already holds the operator's own login token read-only, so masking the surrounding identity would be theatre, and remapping the cwd path would break view↔host path-consistency for no security gain. Masking-by-default is deliberately not the design; the opt-in knob serves deployments whose threat model wants the persona surface reduced.
...
**Residuals.** Three, all recorded rather than papered over: (1) **exfiltration inside an approved flow** — a workload granted `api.example:443` can encode data into its datagrams, the same in-band shape as T1.8 (the framework constrains *where* it connects, not *what* it says); (2) **AF_INET-only legacy clients fail** — a client that calls `gethostbyname`/`getaddrinfo` for an A record gets NODATA (the shim is AAAA-only), so a UDP client that cannot use IPv6 does not work under `[net.udp]`; (3) **the wildcard-label bound is concurrent, not total** — a wildcard grant's synthetic-mint window (32 per grant) rotates: minting a new subdomain past the window evicts the least-recently-used mint that carries no live flow, so a legitimate >32-subdomain fan-out keeps working, but a DNS-label tunnel through one wildcard grant is bounded per *moment* (32 concurrent names, and never more than 32 names with live flows) rather than per *lifetime* — over time it can cycle unlimited distinct labels at DNS-query rate, all still zero-wire at the shim and all inside the granted family. Accepted: the ever-cap it replaces broke real fan-out clients, and the in-band residual (1) already concedes unbounded exfiltration *within* an approved destination. Proxy-aware clients are unaffected (they never attempt raw UDP), so the channel serves only the raw-QUIC / DNS-tooling / VoIP residual population it was built for.
...
**The workload cannot instantiate its own binder surface.** kenneld's node 0 is the *mediated* binder path, but the kernel's binder driver is reachable to anyone who can mount a fresh `binderfs` instance — and on current kernels (the upstream `CONFIG_ANDROID_BINDER_DEVICES` default is now non-empty, `"binder,hwbinder,vndbinder"`, and binderfs auto-populates those devices *per instance* at mount time since Linux 5.4) every user namespace that can mount binderfs is handed a private, pre-populated `binder` context-manager device — a kernel attack surface independent of kenneld's gateway. A confined workload *can* create a nested user namespace (the framework does not deny `CLONE_NEWUSER`) and become root within it, but it cannot mount binderfs there: the `base-confined` seccomp floor denies `mount`/`umount`/`pivot_root`, the entire new-mount API (`fsopen`/`fsconfig`/`fsmount`/`move_mount`/`open_tree`/`mount_setattr`), and `open_by_handle_at`. Every syscall that could instantiate a fresh binderfs instance is a hard seccomp denial, so the only binder the workload reaches is kenneld's policy-mediated node 0. This is a **seccomp-floor** guarantee — non-droppable for `base-confined`'s descendants via the deny-union rule — not the per-uid argument, which a nested userns (uid 0 within it) would defeat.

**Residuals.** The decoder is hardened by fuzzing, not proven; a novel parser bug in `kennel-lib-binder` or a policy-evaluation gap in `kenneld` would be reachable from inside every kennel, since the bus is unavoidable. The blast radius of such a bug is `kenneld`'s privilege (T5.3). Compromise of the kernel binder driver itself — once reached — is out of scope (X7); the mitigation above keeps a *fresh* driver instance unreachable from a confined workload, so the only exposed binder is the hardened gateway.
