# §4.5 Policy surface: D-Bus

D-Bus is proxied, not granted directly. If a context needs D-Bus access, the framework launches an `xdg-dbus-proxy` instance per context that enforces a per-method allowlist between the context and the real bus. The proxy's socket is shimmed into the context as the standard bus path. Without explicit policy enabling it, no bus socket exists in the context's view.

## 4.5.1 Why direct D-Bus access is wrong

A bare socket grant for `$XDG_RUNTIME_DIR/bus` gives the context every D-Bus service the user's session offers. The default session bus has dozens of services connected: notification daemon, network manager, gnome-shell or kwin, file manager, screen lock, login manager, systemd user, secret service, evolution, tracker, packagekit, polkit agent. Each exposes methods that can:
...
Default deny. Anything not listed is rejected with `org.freedesktop.DBus.Error.AccessDenied`. The framework configures the proxy from structured policy and reads the proxy's audit log.

## 4.5.3 How it slots in

Same architectural shape as the SOCKS5 proxy (§4.3). Per context:

```
Host view:
  /run/agent-run/<ctx>/dbus-session.sock
       ← xdg-dbus-proxy listens here
         proxy connects to user's real $DBUS_SESSION_BUS_ADDRESS

Context view (via §4.4 shim):
  $XDG_RUNTIME_DIR/bus
       ← bind-mount from dbus-session.sock
         DBUS_SESSION_BUS_ADDRESS=unix:path=$XDG_RUNTIME_DIR/bus
```

The proxy process runs in the host mount namespace with the real `$DBUS_SESSION_BUS_ADDRESS`. The bind mount exposes its socket to the context as the standard path. Applications open `$XDG_RUNTIME_DIR/bus`, find the proxy, the proxy filters and forwards.

System bus gets a second proxy instance:

```
/run/agent-run/<ctx>/dbus-system.sock
       ← second xdg-dbus-proxy, talks to /var/run/dbus/system_bus_socket
```

Shimmed to `/run/dbus/system_bus_socket` inside the context.

## 4.5.4 Policy primitives
...
[dbus.session.allow]
# Names the context may TALK to (call methods, receive responses)
talk = [
...
call = []
# Broadcast signals the context may receive
broadcast = []
# Names the context may OWN (claim on the bus)
own = []                          # almost always empty for confined contexts
...
[dbus.audit]
log_path = "~/.local/state/agent-run/<context>/dbus.jsonl"
level = "summary"                 # "off" | "summary" | "full"
```

The user writes structured policy. The framework emits the appropriate `--talk=`, `--call=`, `--broadcast=`, `--own=`, `--see=`, `--filter` flags to `xdg-dbus-proxy`. The user does not write proxy command lines directly.

## 4.5.5 The portal pattern
...
- Portal access from non-Flatpak contexts is supported but less battle-tested.
...
For contexts that need portals only and nothing else:
...
## 4.5.6 Notifications: a worked case

The context wants to show desktop notifications (build complete, test failed, AI agent finished). This is one of the simpler legitimate D-Bus grants:
...
- Forward responses.
- Block `NotificationClosed` and `ActionInvoked` signals from other applications (the context only sees signals for its own notifications).
- Block any other service the context might try to reach.

The capability granted: the context can pop notifications. The capabilities not granted: anything else on the session bus.

Worth noting: the notification daemon itself may execute actions on user clicks ("open file", "reply"). The context's notification can specify actions; the user's click triggers them in the user's session, not the context's. This is a path by which the context could trick the user into running something — for instance, a notification action labelled "Click to fix" that actually opens a malicious URL. Templates should consider this carefully; it is not a flaw in the proxy design, it is the inherent property of a user-facing notification capability.

## 4.5.7 What templates should default to

Most confined templates: `dbus.session.enabled = false`. The context has no bus socket; tools that try to connect fail at the shim layer (no socket file there).
...
Templates that need a full desktop integration story (rare for confined contexts): document loudly that this approximates the unconfined session capability and that the threat model is correspondingly weakened.

No templates should grant `org.freedesktop.systemd1` or `org.gnome.SessionManager` to confined contexts. These are categorically too powerful.

## 4.5.8 Operational concerns

**The proxy is a daemon.** One per context (plus a system-bus proxy if enabled). On modern hardware these are inexpensive (~5 MB resident each) but they accumulate if context lifecycles are not managed. Framework supervises lifecycle.

**Bus reconnection.** If the host's dbus-daemon restarts, the proxy reconnects and continues serving the context. The context's bus connection survives the host bus restart because the context's connection terminates at the proxy.
...
**Activation services.** Some D-Bus services are activated on demand (`Type=Activation` units). The proxy must allow `org.freedesktop.DBus.StartServiceByName` for services the context is allowed to talk to. The framework handles this automatically when emitting proxy config.

## 4.5.9 Failure modes

| Situation | Behaviour |
|---|---|
| Proxy crashes | Bus socket in context becomes unresponsive; clients see connection errors. Framework restarts proxy. |
...
3. Same context: `dbus-send --session --dest=org.gnome.SessionManager ...` fails with AccessDenied.
...
7. Proxy survives a restart of the host dbus-daemon (context's bus client may briefly stall but eventually recovers).
